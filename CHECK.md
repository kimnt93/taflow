...
## 0. Registry-driven public-interface verification

Every canonical lifecycle class is discovered by the shared registry in
[`scripts/verification/registry.py`](scripts/verification/registry.py).
TA-Lib mappings come from this file's master table; explicit Wickra fallback
mappings live beside them. Run the complete correctness and lifecycle pass with:

```bash
uv run python scripts/verification/correctness.py
```

The authoritative aggregate is [`verify/CORRECTNESS.md`](verify/CORRECTNESS.md),
with one JSON evidence file per class below `verify/evidence/correctness/`.
`NO_EXTERNAL_ORACLE` is explicit and is never promoted to numerical parity.
...
# TA-Lib/Wickra correctness (writes verify/CORRECTNESS.md)
uv run python scripts/verification/correctness.py

# correctness-gated Python-class benchmark (writes verify/BENCHMARK.md)
uv run python scripts/verification/benchmark.py --quick
# style
