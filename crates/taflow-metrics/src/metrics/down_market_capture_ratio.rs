...
/// Ratio of primary to benchmark annualized growth in down-market periods.
#[derive(Debug, Clone)]
pub struct DownMarketCaptureRatio {
...
impl DownMarketCaptureRatio {
    /// Construct an empty down-market capture state for aligned input domains.
    pub fn new(
        ...
        if matches!(
            ...
            return Err(MetricError::InvalidParameter {
                ...
                reason: "down-market capture ratio requires returns, log returns, equity, or period P&L with initial equity",
            });
        }
    ...
    /// Append one aligned pair and return the current down-market capture ratio.
    ...
    pub fn extend(&mut self, primary: &[f64], benchmark: &[f64]) -> MetricResult<Option<f64>> {
        ...
        let eligible_count = &mut self.eligible_count;
    ...
    /// Return primary down-market CAGR divided by benchmark down-market CAGR.
    ///
    /// At least one eligible pair and a non-zero benchmark CAGR are required.
    pub fn value(&self) -> Option<f64> {
        if self.eligible_count == 0 {
            return None;
        }
        let exponent = self.periods_per_year / self.eligible_count as f64;
    ...
    /// Return the number of pairs whose normalized benchmark return was negative.
