...
fn computes_three_component_product() {
    let mut m =
        CompositeProfitabilityConsistencyIndex::new(MetricInputKind::Returns, NanPolicy::Omit)
            .unwrap();
...
fn trades_and_returns_preserve_values() {
    ...
    let mut a =
        CompositeProfitabilityConsistencyIndex::new(MetricInputKind::Returns, NanPolicy::Omit)
            .unwrap();
    let mut b =
        CompositeProfitabilityConsistencyIndex::new(MetricInputKind::Trades, NanPolicy::Omit)
            .unwrap();
...
fn edges_lifecycle_and_validation() {
    ...
    assert!(
        CompositeProfitabilityConsistencyIndex::new(MetricInputKind::RawPnl, NanPolicy::Omit)
            .is_err()
    );
}
