...
/// Capture ratio restricted to periods with a positive benchmark return.
#[derive(Debug, Clone)]
pub struct UpMarketCaptureRatio {
...
impl UpMarketCaptureRatio {
    /// Construct an empty up-market capture state for aligned input domains.
    pub fn new(
        ...
        if matches!(
            ...
            return Err(MetricError::InvalidParameter {
                ...
                reason: "up-market capture ratio requires returns, log returns, equity, or period P&L with initial equity",
            });
        }
    ...
    /// Append one aligned pair and return the current up-market capture ratio.
        ...
        input.extend_slices(primary, benchmark, |primary_return, benchmark_return| {
            if benchmark_return > 0.0 {
                ...
                benchmark_growth
                    ...
                    .expect("validated positive benchmark return must compound");
            }
        })?;
        Ok(self.value())
    }

    /// Return the ratio of primary CAGR to benchmark CAGR in up-market periods.
    ///
    /// At least one aligned pair whose benchmark return is positive and a
    /// non-zero benchmark annual return are required.
    pub fn value(&self) -> Option<f64> {
        let count = self.primary_growth.len();
        if count == 0 {
            return None;
        }
