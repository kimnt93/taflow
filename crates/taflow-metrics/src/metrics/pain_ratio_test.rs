...
fn computes_pinned_source_definition_and_preserves_lifecycle() {
    ...
    let mut state = PainRatio::new(MetricInputKind::Returns, 12.0, 0.03, NanPolicy::Omit).unwrap();
...
fn all_return_input_modes_are_equivalent() {
    ...
    let mut equity = PainRatio::new(MetricInputKind::Equity, 12.0, 0.02, NanPolicy::Omit).unwrap();
    assert_close(
        equity.extend(&[100.0, 110.0, 88.0, 92.4]).unwrap().unwrap(),
        expected,
    );

    let mut pnl = PainRatio::new(
    ...
    let mut logarithmic =
        PainRatio::new(MetricInputKind::LogReturns, 12.0, 0.02, NanPolicy::Omit).unwrap();
    assert_close(
...
fn handles_missing_values_zero_pain_and_total_loss() {
    let mut state = PainRatio::new(MetricInputKind::Returns, 12.0, 0.0, NanPolicy::Omit).unwrap();
    ...
    let mut no_pain =
        PainRatio::new(MetricInputKind::Returns, 252.0, 0.0, NanPolicy::Omit).unwrap();
    no_pain.extend(&[0.01, 0.02]).unwrap();
    assert_eq!(no_pain.value(), None);

    let mut total_loss =
        PainRatio::new(MetricInputKind::Returns, 1.0, 0.0, NanPolicy::Omit).unwrap();
    assert_eq!(total_loss.append(-1.0).unwrap(), Some(-1.0));
}

#[test]
fn raise_policy_rejects_nan_without_mutating_state() {
    let mut state = PainRatio::new(MetricInputKind::Returns, 12.0, 0.0, NanPolicy::Raise).unwrap();
...
fn validates_configuration_values_and_semantic_domain() {
    for invalid in [0.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(PainRatio::new(MetricInputKind::Returns, invalid, 0.0, NanPolicy::Omit).is_err());
    }
    for invalid in [-1.0, -2.0, f64::NAN, f64::INFINITY] {
        assert!(PainRatio::new(MetricInputKind::Returns, 252.0, invalid, NanPolicy::Omit).is_err());
    }
    assert!(PainRatio::new(MetricInputKind::RawPnl, 252.0, 0.0, NanPolicy::Omit).is_err());
    assert!(PainRatio::new(MetricInputKind::Trades, 252.0, 0.0, NanPolicy::Omit).is_err());

    let mut state = PainRatio::new(MetricInputKind::Returns, 252.0, 0.0, NanPolicy::Omit).unwrap();
