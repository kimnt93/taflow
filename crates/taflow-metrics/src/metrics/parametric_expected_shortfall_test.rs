...
fn computes_gaussian_lower_tail_mean() {
    let mut m =
        ParametricExpectedShortfall::new(MetricInputKind::Returns, 0.05, NanPolicy::Omit).unwrap();
    let actual = m
...
#[test]
fn factories_lifecycle_and_missing_are_invariant() {
    ...
    let mut a =
        ParametricExpectedShortfall::new(MetricInputKind::Returns, 0.05, NanPolicy::Omit).unwrap();
    let mut b =
        ParametricExpectedShortfall::new(MetricInputKind::LogReturns, 0.05, NanPolicy::Omit)
            .unwrap();
    let mut c =
        ParametricExpectedShortfall::new(MetricInputKind::Equity, 0.05, NanPolicy::Omit).unwrap();
    let mut d = ParametricExpectedShortfall::new(
...
fn validates_cutoff_domain_and_warmup() {
    assert!(
        ParametricExpectedShortfall::new(MetricInputKind::Returns, 0., NanPolicy::Omit).is_err()
    );
    assert!(
        ParametricExpectedShortfall::new(MetricInputKind::Trades, 0.05, NanPolicy::Omit).is_err()
    );
    let mut m =
        ParametricExpectedShortfall::new(MetricInputKind::Returns, 0.05, NanPolicy::Omit).unwrap();
