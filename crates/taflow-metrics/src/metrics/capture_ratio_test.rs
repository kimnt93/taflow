...
fn computes_geometric_annual_return_ratio_and_preserves_lifecycle() {
    ...
    let mut state = CaptureRatio::new(252.0, NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[], &[])?;
            Ok(state)
        })
...
fn omits_missing_values_pairwise_and_handles_zero_benchmark_growth() {
    let mut state = CaptureRatio::new(12.0, NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[], &[])?;
            Ok(state)
        })
    ...
    let mut flat = CaptureRatio::new(252.0, NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[], &[])?;
            Ok(state)
        })
...
fn input_modes_produce_equivalent_capture_ratio() {
    ...
    let mut equity = CaptureRatio::new(12.0, NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_equity(&[], &[])?;
            Ok(state)
        })
    ...
    let mut pnl = CaptureRatio::new(12.0, NanPolicy::Omit)
        .and_then(|mut state| {
    ...
    let mut logarithmic = CaptureRatio::new(12.0, NanPolicy::Omit)
        .and_then(|mut state| {
...
fn rejects_invalid_configuration_misalignment_and_domains() {
    for invalid in [0.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(CaptureRatio::new(invalid, NanPolicy::Omit)
            .and_then(|mut state| {
                state.from_returns(&[], &[])?;
                Ok(state)
            })
    ...
    assert!(CaptureRatio::new(252.0, NanPolicy::Omit)
        .and_then(|mut state| {
