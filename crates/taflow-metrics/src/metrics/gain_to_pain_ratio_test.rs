...
fn computes_frozen_quantstats_definition_and_preserves_lifecycle() {
    ...
    let mut state = GainToPainRatio::new(NanPolicy::Omit).unwrap();
...
fn all_return_input_modes_are_equivalent() {
    ...
    let mut equity = GainToPainRatio::new(NanPolicy::Omit).unwrap();
    ...
    let mut pnl = GainToPainRatio::new(NanPolicy::Omit).unwrap();
    ...
    let mut logarithmic = GainToPainRatio::new(NanPolicy::Omit).unwrap();
...
fn handles_missing_values_and_zero_pain() {
    ...
    let mut no_losses = GainToPainRatio::new(NanPolicy::Omit).unwrap();
    no_losses.from_returns(&[]).unwrap();
    no_losses.extend(&[0.01, 0.02, 0.0]).unwrap();
    assert_eq!(no_losses.value(), None);

    let mut only_losses = GainToPainRatio::new(NanPolicy::Omit).unwrap();
    only_losses.from_returns(&[]).unwrap();
...
fn raise_policy_rejects_nan_without_mutating_state() {
    let mut state = GainToPainRatio::new(NanPolicy::Raise).unwrap();
    state.from_returns(&[]).unwrap();
...
fn validates_values_and_semantic_domain() {
    let mut unbound = GainToPainRatio::new(NanPolicy::Omit).unwrap();
