...
fn computes_linear_tail_quantiles_and_refreshes_dirty_cache() {
    ...
    let mut state = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn scalar_chunked_and_batch_updates_are_invariant() {
    ...
    let mut scalar = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
    ...
    let mut chunked = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
    ...
    let mut batch = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn input_modes_produce_equivalent_ratios() {
    ...
    let mut equity = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_equity(&[])?;
            Ok(state)
        })
    ...
    let mut pnl = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
    ...
    let mut logarithmic = TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
...
fn handles_warmup_missing_values_zero_denominator_and_reset() {
    ...
    let mut raising = TailRatio::new(NanPolicy::Raise)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn rejects_non_return_domains() {
    assert!(TailRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.append(0.0)?;
            Ok(state)
        })
