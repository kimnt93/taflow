...
fn computes_empyrical_definition_and_preserves_lifecycle() {
    ...
    let mut state =
        SortinoRatio::new(MetricInputKind::Returns, 12.0, 0.06, NanPolicy::Omit).unwrap();
...
fn all_return_input_modes_are_equivalent() {
    ...
    let mut equity =
        SortinoRatio::new(MetricInputKind::Equity, 12.0, 0.03, NanPolicy::Omit).unwrap();
    assert_close(
    ...
    let mut logarithmic =
        SortinoRatio::new(MetricInputKind::LogReturns, 12.0, 0.03, NanPolicy::Omit).unwrap();
...
fn handles_missing_minimum_sample_and_zero_downside() {
    let mut state =
        SortinoRatio::new(MetricInputKind::Returns, 252.0, 0.0, NanPolicy::Omit).unwrap();
    ...
    let mut no_downside =
        SortinoRatio::new(MetricInputKind::Returns, 252.0, 0.0, NanPolicy::Omit).unwrap();
    no_downside.extend(&[0.01, 0.02]).unwrap();
...
fn validates_configuration_and_semantic_domain() {
    for invalid in [0.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(
            SortinoRatio::new(MetricInputKind::Returns, invalid, 0.0, NanPolicy::Omit).is_err()
        );
    }
    for invalid in [-1.0, -2.0, f64::NAN, f64::INFINITY] {
        assert!(
            SortinoRatio::new(MetricInputKind::Returns, 252.0, invalid, NanPolicy::Omit).is_err()
        );
    }
    assert!(SortinoRatio::new(MetricInputKind::RawPnl, 252.0, 0.0, NanPolicy::Omit).is_err());
    assert!(SortinoRatio::new(MetricInputKind::Trades, 252.0, 0.0, NanPolicy::Omit).is_err());
}
