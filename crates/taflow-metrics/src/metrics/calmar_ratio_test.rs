...
fn computes_geometric_annual_return_over_absolute_drawdown() {
    ...
    let mut metric = CalmarRatio::new(12.0, NanPolicy::Omit).unwrap();
...
fn semantic_input_modes_and_streaming_are_invariant() {
    ...
    let mut expected = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    expected.from_returns(&[]).unwrap();
    expected.extend(&returns).unwrap();
    let expected_value = expected.value().unwrap();

    let mut from_equity = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    from_equity.from_equity(&[]).unwrap();
    from_equity.extend(&equity).unwrap();
    assert_close(from_equity.value().unwrap(), expected_value);

    let mut from_pnl = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    from_pnl.from_pnl(&[], 100.0).unwrap();
    from_pnl.extend(&pnl).unwrap();
    assert_close(from_pnl.value().unwrap(), expected_value);

    let mut from_log = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    from_log.from_log_returns(&[]).unwrap();
    from_log.extend(&log_returns).unwrap();
    assert_close(from_log.value().unwrap(), expected_value);

    let mut streamed = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
...
fn undefined_and_missing_value_contract_is_explicit() {
    let mut empty = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    empty.from_returns(&[]).unwrap();
    assert_eq!(empty.value(), None);
    empty.extend(&[0.10, 0.0, 0.20]).unwrap();
    assert_eq!(empty.value(), None);

    let mut omitted = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    omitted.from_returns(&[]).unwrap();
    omitted.extend(&[f64::NAN, -0.10, f64::NAN]).unwrap();
    assert_eq!(omitted.len(), 1);
    assert!(omitted.value().is_some());

    let mut total_loss = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    total_loss.from_returns(&[]).unwrap();
...
fn rejects_invalid_configuration_and_observations() {
    for periods_per_year in [0.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(CalmarRatio::new(periods_per_year, NanPolicy::Omit).is_err());
    }
    let mut unbound = CalmarRatio::new(252.0, NanPolicy::Omit).unwrap();
    assert!(unbound.append(0.01).is_err());

    let mut state = CalmarRatio::new(252.0, NanPolicy::Raise).unwrap();
