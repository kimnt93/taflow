...
fn freezes_quantstats_compounded_unannualized_definition() {
    ...
    let mut metric = UlcerPerformanceIndex::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
...
fn semantic_input_modes_and_streaming_are_invariant() {
    ...
    let mut expected =
        UlcerPerformanceIndex::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
    expected.extend(&returns).unwrap();
    let expected_value = expected.value().unwrap();

    let mut from_equity =
        UlcerPerformanceIndex::new(MetricInputKind::Equity, NanPolicy::Omit).unwrap();
    from_equity.extend(&equity).unwrap();
    assert_close(from_equity.value().unwrap(), expected_value);

    let mut from_pnl = UlcerPerformanceIndex::new(
    ...
    let mut from_log =
        UlcerPerformanceIndex::new(MetricInputKind::LogReturns, NanPolicy::Omit).unwrap();
...
fn warmup_zero_risk_and_missing_contract_is_explicit() {
    let mut state = UlcerPerformanceIndex::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
...
fn rejects_ineligible_domains_and_invalid_observations() {
    assert!(UlcerPerformanceIndex::new(MetricInputKind::RawPnl, NanPolicy::Omit).is_err());
    assert!(UlcerPerformanceIndex::new(MetricInputKind::Trades, NanPolicy::Omit).is_err());

    let mut state = UlcerPerformanceIndex::new(MetricInputKind::Returns, NanPolicy::Raise).unwrap();
