...
fn matches_profit_factor_times_exact_tail_ratio() {
    ...
    let mut state = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn freezes_zero_denominator_and_loss_only_edges() {
    let mut positive = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
    ...
    let mut losses = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
    ...
    let mut zero = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn lifecycle_chunking_omission_and_reset_are_invariant() {
    ...
    let mut batch = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
    ...
    let mut streamed = CommonSenseRatio::new(NanPolicy::Omit)
        .and_then(|mut state| {
            state.from_returns(&[])?;
            Ok(state)
        })
...
fn rejects_every_non_return_domain() {
    for input_kind in [
        ...
        let _ = input_kind;
        let mut state = CommonSenseRatio::new(NanPolicy::Omit).unwrap();
