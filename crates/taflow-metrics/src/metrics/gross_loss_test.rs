...
fn sums_strictly_negative_period_pnl_and_preserves_lifecycle() {
    ...
    let mut state = GrossLoss::new(MetricInputKind::RawPnl, NanPolicy::Omit).unwrap();
...
fn closed_trade_pnl_uses_the_same_unconverted_signed_sum() {
    ...
    let mut state = GrossLoss::new(MetricInputKind::Trades, NanPolicy::Omit).unwrap();
...
fn missing_and_invalid_values_follow_the_input_contract() {
    let mut omit = GrossLoss::new(MetricInputKind::RawPnl, NanPolicy::Omit).unwrap();
    omit.extend(&[f64::NAN, -1000.0, 50.0]).unwrap();
    assert_eq!(omit.len(), 2);
    assert_eq!(omit.value(), Some(-1000.0));

    let mut raise = GrossLoss::new(MetricInputKind::Trades, NanPolicy::Raise).unwrap();
...
fn rejects_returns_and_return_conversion_domains() {
    assert!(GrossLoss::new(MetricInputKind::Returns, NanPolicy::Omit).is_err());
    assert!(GrossLoss::new(MetricInputKind::LogReturns, NanPolicy::Omit).is_err());
    assert!(GrossLoss::new(MetricInputKind::Equity, NanPolicy::Omit).is_err());
    assert!(GrossLoss::new(
