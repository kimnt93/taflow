...
fn counts_performanceanalytics_drawdown_episode_lengths() {
    let mut metric =
        MaximumDrawdownDuration::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
...
fn factories_and_lifecycle_are_invariant() {
    ...
    let mut a = MaximumDrawdownDuration::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
    let mut b = MaximumDrawdownDuration::new(MetricInputKind::LogReturns, NanPolicy::Omit).unwrap();
    let mut c = MaximumDrawdownDuration::new(MetricInputKind::Equity, NanPolicy::Omit).unwrap();
    let mut d = MaximumDrawdownDuration::new(
...
fn rejects_non_path_domains() {
    assert!(MaximumDrawdownDuration::new(MetricInputKind::Trades, NanPolicy::Omit).is_err());
}
