...
fn compounds_returns_and_preserves_lifecycle() {
    ...
    let mut state = TotalReturn::new(MetricInputKind::Returns, NanPolicy::Omit).unwrap();
...
fn input_modes_produce_equivalent_returns() {
    ...
    let mut equity = TotalReturn::new(MetricInputKind::Equity, NanPolicy::Omit).unwrap();
    assert_close(
    ...
    let mut logarithmic = TotalReturn::new(MetricInputKind::LogReturns, NanPolicy::Omit).unwrap();
...
fn rejects_non_return_semantic_domains() {
    assert!(TotalReturn::new(MetricInputKind::RawPnl, NanPolicy::Omit).is_err());
    assert!(TotalReturn::new(MetricInputKind::Trades, NanPolicy::Omit).is_err());
}
