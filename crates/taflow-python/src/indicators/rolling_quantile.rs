...
#[pyclass]
pub struct RollingQuantileOperator { inner: RollingQuantile, outputs: Vec<f64> }
#[pymethods]
impl RollingQuantileOperator {
    #[new]
    fn new(timeperiod: usize, quantile: f64) -> PyResult<Self> { Ok(Self { inner: RollingQuantile::new(timeperiod, quantile).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
