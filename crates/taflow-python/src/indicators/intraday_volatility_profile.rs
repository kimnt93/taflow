...
use pyo3::prelude::*;
use taflow::indicators::IntradayVolatilityProfile as State;
...
impl IntradayVolatilityProfile {
    ...
    fn append(
        ...
        match &row {
            ...
            None => self
                .output
                .resize(self.output.len() + self.buckets, f64::NAN),
        }
        row
    }
    ...
    fn compute<'py>(&self, py: Python<'py>) -> Bound<'py, PyArray2<f64>> {
        let rows = self.output.len() / self.buckets;
        let array = Array2::from_shape_vec((rows, self.buckets), self.output.clone())
            .expect("profile history has a stable column count");
