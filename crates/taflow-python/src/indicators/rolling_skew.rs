...
#[pyclass]
pub struct RollingSkewOperator { inner: RollingSkew, outputs: Vec<f64> }
#[pymethods]
impl RollingSkewOperator {
    #[new] fn new(timeperiod: usize) -> PyResult<Self> { Ok(Self { inner: RollingSkew::new(timeperiod).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
