...
use pyo3::prelude::*;
use taflow::stream::RollingRank;

#[pyclass]
pub struct RollingRankOperator { inner: RollingRank, outputs: Vec<f64> }
#[pymethods]
impl RollingRankOperator {
    #[new]
    fn new(timeperiod: usize) -> PyResult<Self> { Ok(Self { inner: RollingRank::new(timeperiod).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
