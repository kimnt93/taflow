...
#[pyclass]
pub struct RollingKurtosisOperator { inner: RollingKurtosis, outputs: Vec<f64> }
#[pymethods]
impl RollingKurtosisOperator {
    #[new] fn new(timeperiod: usize) -> PyResult<Self> { Ok(Self { inner: RollingKurtosis::new(timeperiod).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
