...
#[pyclass]
pub struct RollingPercentileOperator { inner: RollingQuantile, outputs: Vec<f64> }
#[pymethods]
impl RollingPercentileOperator {
    #[new]
    fn new(timeperiod: usize, percentile: f64) -> PyResult<Self> { Ok(Self { inner: RollingQuantile::new(timeperiod, percentile / 100.0).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
