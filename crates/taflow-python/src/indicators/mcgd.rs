...
use pyo3::prelude::*;
use taflow::stream::Mcgd;
#[pyclass]
pub struct McgdOperator { inner: Mcgd, values: Vec<f64> }
#[pymethods]
impl McgdOperator {
    #[new]
    #[pyo3(signature = (length=10, c=1.0))]
    fn new(length: usize, c: f64) -> PyResult<Self> { Ok(Self { inner: Mcgd::new(length,c).map_err(|e|PyValueError::new_err(e.to_string()))?, values:Vec::new() }) }
