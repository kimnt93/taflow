...
#[pyclass]
pub struct RollingWinsorizeOperator { inner: RollingWinsorize, outputs: Vec<f64> }
#[pymethods]
impl RollingWinsorizeOperator {
    #[new] fn new(timeperiod: usize, lower: f64, upper: f64) -> PyResult<Self> { Ok(Self { inner: RollingWinsorize::new(timeperiod, lower, upper).map_err(|e| PyValueError::new_err(e.to_string()))?, outputs: Vec::new() }) }
