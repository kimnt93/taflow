...
use pyo3::prelude::*;
use taflow_metrics::{metrics::SystemQualityNumber as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported SystemQualityNumber input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.SystemQualityNumber`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct SystemQualityNumber {
    inner: State,
}
