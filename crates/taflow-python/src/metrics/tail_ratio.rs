...
use pyo3::prelude::*;
use taflow_metrics::{metrics::TailRatio as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported TailRatio input mode")),
    }
}

/// Native state backing `taflow.metrics.TailRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct TailRatio {
    inner: State,
}
