...
use pyo3::prelude::*;
use taflow_metrics::{metrics::AnnualizedVolatility as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported AnnualizedVolatility input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.AnnualizedVolatility`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct AnnualizedVolatility {
    inner: State,
}
...
impl AnnualizedVolatility {
    #[new]
    #[pyo3(signature = (input_mode, periods_per_year=252.0, initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
