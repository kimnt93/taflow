...
use pyo3::prelude::*;
use taflow_metrics::{metrics::BreakevenRate as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported BreakevenRate input mode",
        )),
    }
}
/// Native state backing `taflow.metrics.BreakevenRate`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct BreakevenRate {
    inner: State,
}
#[pymethods]
impl BreakevenRate {
