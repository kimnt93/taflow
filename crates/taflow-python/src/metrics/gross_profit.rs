...
use pyo3::prelude::*;
use taflow_metrics::{metrics::GrossProfit as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported GrossProfit input mode")),
    }
}

/// Native state backing `taflow.metrics.GrossProfit`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct GrossProfit {
    inner: State,
}
