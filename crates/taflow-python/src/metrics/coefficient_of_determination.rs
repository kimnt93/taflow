...
use pyo3::prelude::*;
use taflow_metrics::{metrics::CoefficientOfDetermination as State, MetricInputKind, NanPolicy};
...
fn input_kinds(
    ...
    match (name, initial_equity, benchmark_initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported CoefficientOfDetermination input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.CoefficientOfDetermination`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct CoefficientOfDetermination {
    inner: State,
}
...
impl CoefficientOfDetermination {
    #[new]
    #[pyo3(signature = (input_mode, initial_equity=None, benchmark_initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
        initial_equity: Option<f64>,
        benchmark_initial_equity: Option<f64>,
