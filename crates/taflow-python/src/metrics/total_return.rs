...
use pyo3::prelude::*;
use taflow_metrics::{metrics::TotalReturn as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported TotalReturn input mode")),
    }
}

/// Native state backing `taflow.metrics.TotalReturn`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct TotalReturn {
    inner: State,
}
