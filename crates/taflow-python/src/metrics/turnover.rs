...
use pyo3::prelude::*;
use taflow_metrics::{metrics::Turnover as State, NanPolicy};

fn value_error(error: impl ToString) -> PyErr {
    PyValueError::new_err(error.to_string())
}

/// Native state backing `taflow.metrics.Turnover`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct Turnover {
    inner: State,
}
...
impl Turnover {
    ...
    fn extend(&mut self, py: Python<'_>, weights: PyReadonlyArray1<'_, f64>) -> PyResult<()> {
        let weights = weights.as_slice()?;
        py.allow_threads(|| self.inner.extend(weights))
            .map(|_| ())
            .map_err(value_error)
    }
