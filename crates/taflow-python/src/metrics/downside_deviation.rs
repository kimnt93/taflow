...
use pyo3::prelude::*;
use taflow_metrics::{metrics::DownsideDeviation as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported DownsideDeviation input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.DownsideDeviation`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct DownsideDeviation {
    inner: State,
}
...
impl DownsideDeviation {
    #[new]
    #[pyo3(signature = (input_mode, periods_per_year=252.0, annual_required_return=0.0, initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
