use numpy::PyReadonlyArray1;
use pyo3::{exceptions::PyValueError, prelude::*, types::PyDict};
use taflow_metrics::{
    MetricInputKind, MetricPipeline as State, MetricPipelineConfiguration, NanPolicy,
};

fn value_error(error: impl ToString) -> PyErr {
    PyValueError::new_err(error.to_string())
}

fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ("returns", None) => Ok(MetricInputKind::Returns),
        ("log_returns", None) => Ok(MetricInputKind::LogReturns),
        ("equity", None) => Ok(MetricInputKind::Equity),
        ("pnl", Some(initial_equity)) => Ok(MetricInputKind::PeriodPnl { initial_equity }),
        ("pnl", None) => Err(PyValueError::new_err(
            "initial_equity is required for period-P&L conversion",
        )),
        (_, Some(_)) => Err(PyValueError::new_err(
            "initial_equity is accepted only for period-P&L input",
        )),
        _ => Err(PyValueError::new_err(
            "unsupported MetricPipeline input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.MetricPipeline`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct MetricPipeline {
    inner: State,
}

impl MetricPipeline {
    fn result_dict(&mut self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        let result = PyDict::new(py);
        for (name, value) in self.inner.compute() {
            result.set_item(name, value)?;
        }
        Ok(result.unbind())
    }
}

#[pymethods]
impl MetricPipeline {
    #[new]
    #[pyo3(signature = (
        input_mode,
        metrics,
        periods_per_year=252.0,
        annual_risk_free_rate=0.0,
        annual_required_return=0.0,
        annual_benchmark_sharpe_ratio=0.0,
        cutoff=0.05,
        confidence_level=0.95,
        initial_equity=None,
        nan_policy="omit"
    ))]
    #[allow(clippy::too_many_arguments)]
    fn new(
        input_mode: &str,
        metrics: Vec<String>,
        periods_per_year: f64,
        annual_risk_free_rate: f64,
        annual_required_return: f64,
        annual_benchmark_sharpe_ratio: f64,
        cutoff: f64,
        confidence_level: f64,
        initial_equity: Option<f64>,
        nan_policy: &str,
    ) -> PyResult<Self> {
        let configuration = MetricPipelineConfiguration {
            periods_per_year,
            annual_risk_free_rate,
            annual_required_return,
            annual_benchmark_sharpe_ratio,
            cutoff,
            confidence_level,
        };
        Ok(Self {
            inner: State::new(
                input_kind(input_mode, initial_equity)?,
                &metrics,
                configuration,
                NanPolicy::try_from(nan_policy).map_err(value_error)?,
            )
            .map_err(value_error)?,
        })
    }

    #[staticmethod]
    fn supported_metrics() -> Vec<&'static str> {
        State::SUPPORTED_METRICS.to_vec()
    }

    #[getter]
    fn metrics(&self) -> Vec<&'static str> {
        self.inner.metric_names()
    }

    fn append(&mut self, value: f64) -> PyResult<()> {
        self.inner.append(value).map_err(value_error)
    }

    fn extend(&mut self, py: Python<'_>, values: PyReadonlyArray1<'_, f64>) -> PyResult<()> {
        let values = values.as_slice()?;
        py.allow_threads(|| self.inner.extend(values))
            .map_err(value_error)
    }

    #[getter]
    fn value(&mut self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        self.result_dict(py)
    }

    fn compute(&mut self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        self.result_dict(py)
    }

    fn reset(&mut self) {
        self.inner.reset();
    }

    fn __len__(&self) -> usize {
        self.inner.len()
    }
}
