...
use pyo3::prelude::*;
use taflow_metrics::{metrics::MaximumDrawdown as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported MaximumDrawdown input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.MaximumDrawdown`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct MaximumDrawdown {
    inner: State,
}
