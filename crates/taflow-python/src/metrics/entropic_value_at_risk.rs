...
use pyo3::prelude::*;
use taflow_metrics::{metrics::EntropicValueAtRisk as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported EntropicValueAtRisk input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.EntropicValueAtRisk`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct EntropicValueAtRisk {
    inner: State,
}
...
impl EntropicValueAtRisk {
    ...
    #[getter]
    fn value(&mut self, py: Python<'_>) -> Option<f64> {
        py.allow_threads(|| self.inner.value())
    }

    fn compute(&mut self, py: Python<'_>) -> Option<f64> {
