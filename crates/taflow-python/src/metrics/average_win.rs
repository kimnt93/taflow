...
use pyo3::prelude::*;
use taflow_metrics::{metrics::AverageWin as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported AverageWin input mode")),
    }
}

/// Native state backing `taflow.metrics.AverageWin`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct AverageWin {
    inner: State,
}
