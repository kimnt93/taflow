...
use pyo3::prelude::*;
use taflow_metrics::{metrics::ProfitFactor as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported ProfitFactor input mode")),
    }
}

/// Native state backing `taflow.metrics.ProfitFactor`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct ProfitFactor {
    inner: State,
}
