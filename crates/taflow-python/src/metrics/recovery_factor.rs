...
use pyo3::prelude::*;
use taflow_metrics::{metrics::RecoveryFactor as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported RecoveryFactor input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.RecoveryFactor`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct RecoveryFactor {
    inner: State,
}
