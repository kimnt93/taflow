...
use pyo3::prelude::*;
use taflow_metrics::{metrics::PayoffRatio as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported PayoffRatio input mode")),
    }
}

/// Native state backing `taflow.metrics.PayoffRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct PayoffRatio {
    inner: State,
}
