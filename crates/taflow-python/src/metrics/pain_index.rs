...
use pyo3::prelude::*;
use taflow_metrics::{metrics::PainIndex as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported PainIndex input mode")),
    }
}

/// Native state backing `taflow.metrics.PainIndex`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct PainIndex {
    inner: State,
}
