...
use pyo3::prelude::*;
use taflow_metrics::{metrics::TreynorRatio as State, MetricInputKind, NanPolicy};
...
fn input_kinds(
    ...
    match (name, initial_equity, benchmark_initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported TreynorRatio input mode")),
    }
}

/// Native state backing `taflow.metrics.TreynorRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct TreynorRatio {
    inner: State,
}
...
impl TreynorRatio {
    #[new]
    #[pyo3(signature = (input_mode, periods_per_year=252.0, annual_risk_free_rate=0.0, initial_equity=None, benchmark_initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
