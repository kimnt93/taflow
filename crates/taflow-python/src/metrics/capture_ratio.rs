...
use pyo3::prelude::*;
use taflow_metrics::{metrics::CaptureRatio as State, MetricInputKind, NanPolicy};
...
fn input_kinds(
    ...
    match (name, initial_equity, benchmark_initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported CaptureRatio input mode")),
    }
}

/// Native state backing `taflow.metrics.CaptureRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct CaptureRatio {
    inner: State,
}
