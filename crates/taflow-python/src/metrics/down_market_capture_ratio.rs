...
use pyo3::prelude::*;
use taflow_metrics::{metrics::DownMarketCaptureRatio as State, MetricInputKind, NanPolicy};
...
fn input_kinds(
    ...
    match (name, initial_equity, benchmark_initial_equity) {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported DownMarketCaptureRatio input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.DownMarketCaptureRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct DownMarketCaptureRatio {
    inner: State,
}
...
impl DownMarketCaptureRatio {
    #[new]
    #[pyo3(signature = (input_mode, periods_per_year=252.0, initial_equity=None, benchmark_initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
