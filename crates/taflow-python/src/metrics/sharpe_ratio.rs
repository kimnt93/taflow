...
use pyo3::prelude::*;
use taflow_metrics::{metrics::SharpeRatio as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str, initial_equity: Option<f64>) -> PyResult<MetricInputKind> {
    match (name, initial_equity) {
        ...
        _ => Err(PyValueError::new_err("unsupported SharpeRatio input mode")),
    }
}

/// Native state backing `taflow.metrics.SharpeRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct SharpeRatio {
    inner: State,
}
...
impl SharpeRatio {
    #[new]
    #[pyo3(signature = (input_mode, periods_per_year=252.0, annual_risk_free_rate=0.0, initial_equity=None, nan_policy="omit"))]
    fn new(
        input_mode: &str,
