...
use pyo3::prelude::*;
use taflow_metrics::{metrics::InformationRatio as State, NanPolicy};

fn value_error(error: impl ToString) -> PyErr {
    PyValueError::new_err(error.to_string())
}
/// Native state backing `taflow.metrics.InformationRatio`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct InformationRatio {
    inner: State,
}
...
impl InformationRatio {
    #[new]
    #[pyo3(signature = (periods_per_year=252.0, annualized=true, nan_policy="omit"))]
    fn new(periods_per_year: f64, annualized: bool, nan_policy: &str) -> PyResult<Self> {
        Ok(Self {
            inner: State::new(
                periods_per_year,
                annualized,
