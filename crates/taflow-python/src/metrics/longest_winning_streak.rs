...
use pyo3::prelude::*;
use taflow_metrics::{metrics::LongestWinningStreak as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err(
            "unsupported LongestWinningStreak input mode",
        )),
    }
}

/// Native state backing `taflow.metrics.LongestWinningStreak`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct LongestWinningStreak {
    inner: State,
}
