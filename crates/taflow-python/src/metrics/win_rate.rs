...
use pyo3::prelude::*;
use taflow_metrics::{metrics::WinRate as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported WinRate input mode")),
    }
}

/// Native state backing `taflow.metrics.WinRate`.
