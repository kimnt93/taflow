...
use pyo3::prelude::*;
use taflow_metrics::{metrics::AverageLoss as State, MetricInputKind, NanPolicy};
...
fn input_kind(name: &str) -> PyResult<MetricInputKind> {
    match name {
        ...
        _ => Err(PyValueError::new_err("unsupported AverageLoss input mode")),
    }
}

/// Native state backing `taflow.metrics.AverageLoss`.
#[pyclass(module = "taflow._native.metrics")]
pub(crate) struct AverageLoss {
    inner: State,
}
