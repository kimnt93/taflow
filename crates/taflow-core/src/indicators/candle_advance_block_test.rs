use super::candle_advance_block::CandleAdvanceBlock;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleAdvanceBlock::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_matches_scalar_at_every_split_and_after_continuation() {
    ...
    let low: Vec<_> = open
        ...
        .collect();
    let mut scalar = CandleAdvanceBlock::new();
    ...
    for split in 0..=len {
        let mut state = CandleAdvanceBlock::new();
