use super::candle_morning_doji_star::CandleMorningDojiStar;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleMorningDojiStar::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_matches_scalar_at_every_split_and_after_continuation() {
    ...
    let low: Vec<_> = open
        ...
        .collect();
    let mut scalar = CandleMorningDojiStar::new();
    ...
    for split in 0..=len {
        let mut state = CandleMorningDojiStar::new();
