use super::candle_harami_cross::CandleHaramiCross;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleHaramiCross::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_all_splits_and_continuation_match_scalar_replay() {
    ...
    let low: Vec<_> = open
        ...
        .collect();
    let mut scalar = CandleHaramiCross::new();
    ...
    for split in 0..=open.len() {
        let mut state = CandleHaramiCross::new();
