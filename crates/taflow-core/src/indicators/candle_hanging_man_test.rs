use super::candle_hanging_man::CandleHangingMan;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleHangingMan::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_all_splits_and_continuation_match_scalar_replay() {
    ...
    let low: Vec<_> = open
        ...
        .collect();
    let mut scalar = CandleHangingMan::new();
    ...
    for split in 0..=open.len() {
        let mut state = CandleHangingMan::new();
