...
impl CandlePiercing {
    ...
    pub fn extend_slices_into(
        ...
        let mut body_prev_sum = open[..10]
            .iter()
            .zip(&close[..10])
            .fold(0.0, |sum, (&o, &c)| sum + cr_realbody_scalar(o, c));
        let mut body_cur_sum = open[1..11]
            .iter()
            .zip(&close[1..11])
            .fold(0.0, |sum, (&o, &c)| sum + cr_realbody_scalar(o, c));
        let start = output.len();
        output.resize(start + len, 0);
        for (((opens, lows), closes), out) in open
            .windows(12)
            .zip(low.windows(12))
            ...
            .zip(output[start + LOOKBACK..].iter_mut())
        {
            ...
            let long_prev = ca_realbody_scalar(BODY_LONG, body_prev_sum, prev.o, prev.c);
            let long_cur = ca_realbody_scalar(BODY_LONG, body_cur_sum, cur.o, cur.c);
            *out = (prev.color() == -1
                && prev.body() > long_prev
                && cur.color() == 1
                && cur.body() > long_cur
        ...
        self.body_sum = [body_cur_sum, body_prev_sum];
