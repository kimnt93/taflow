use super::triangular_moving_average::TriangularMovingAverage;
...
fn scalar_bulk_chunks_continuation_and_reset_are_bitwise_invariant() {
    ...
    for period in [1, 2, 3, 9, 10, 30] {
        let mut scalar = TriangularMovingAverage::new(period).unwrap();
        let expected: Vec<f64> = input
            .iter()
            .map(|&value| scalar.append(value).unwrap_or(f64::NAN))
            .collect();

        let mut bulk = TriangularMovingAverage::new(period).unwrap();
        ...
        let mut chunked = TriangularMovingAverage::new(period).unwrap();
