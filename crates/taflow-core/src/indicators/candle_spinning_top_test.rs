use super::candle_spinning_top::CandleSpinningTop;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleSpinningTop::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_is_bitwise_identical_for_every_chunk_split_and_continuation() {
    ...
    let mut scalar = CandleSpinningTop::new();
    ...
    for split in 0..=len {
        let mut chunked = CandleSpinningTop::new();
