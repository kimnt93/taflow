use super::candle_separating_lines::CandleSeparatingLines;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleSeparatingLines::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_all_splits_and_continuation_match_scalar_replay() {
    ...
    let high: Vec<_> = open
        .iter()
        .zip(&close)
        .map(|(&open, &close)| open.max(close) + 0.6)
        .collect();
    let low: Vec<_> = open
        .iter()
        .zip(&close)
        ...
        .collect();
    let mut scalar = CandleSeparatingLines::new();
    ...
    for split in 0..=open.len() {
        let mut state = CandleSeparatingLines::new();
