use super::candle_kicking::CandleKicking;

#[test]
fn lifecycle_and_reset_are_causal() {
    let mut state = CandleKicking::default();
    for index in 0..12 {
        state.append(
            100.0 + index as f64,
            102.0 + index as f64,
...
fn bulk_is_bitwise_identical_for_every_chunk_split_and_continuation() {
    ...
    let mut scalar = CandleKicking::new();
    ...
    let continuation = (96.7, 97.2, 95.8, 96.1);
    ...
    for split in 0..=len {
        let mut chunked = CandleKicking::new();
