use super::math_log10::MathLog10;

#[test]
fn lifecycle_and_bulk_are_consistent() {
    let input = [-0.75_f64, -0.25, 0.25, 0.75];
    let expected: Vec<_> = input.iter().map(|&input| input.log10()).collect();
    let mut state = MathLog10::new().unwrap();
...
fn repeated_bulk_and_scalar_continuation_preserve_exact_state() {
    let input = [-1.0_f64, -0.0, 0.0, 0.1, 1.0, 10.0, f64::INFINITY, f64::NAN];
    let mut scalar = MathLog10::new().unwrap();
    let expected: Vec<_> = input
        .iter()
        .map(|&value| scalar.append(value).unwrap())
        .collect();
    let expected_next = scalar.append(100.0);

    for split in 0..=input.len() {
        let mut chunked = MathLog10::new().unwrap();
        ...
        assert_eq!(
            chunked.append(100.0).map(f64::to_bits),
