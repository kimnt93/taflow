use super::ultimate_oscillator::UltimateOscillator;
...
fn scalar_bulk_and_reset_are_invariant() {
    ...
    let mut scalar = UltimateOscillator::new(7, 14, 28).unwrap();
    let scalar_out: Vec<f64> = high
        ...
        .collect();
    let mut bulk = UltimateOscillator::new(7, 14, 28).unwrap();
