...
impl CandleUpsideGapTwoCrows {
    ...
    pub fn extend_slices_into(
        ...
        let scores = candle_upside_gap_two_crows(open, high, low, close)?;
...
pub fn candle_upside_gap_two_crows(
    open: &[f64],
    high: &[f64],
    low: &[f64],
    close: &[f64],
    ...
    for i in start..len {
        output[i] = (candle_color(open[i-2], close[i-2]) == 1
            ...
            && real_body(open[i-1], close[i-1]) <= ca_realbody(BODY_SHORT, body_short_sum, open, high, low, close, i-1)
            && real_body_gap_up(open, close, i-1, i-2)
            // 3rd: black, engulfs 2nd body, close above 1st close
            && candle_color(open[i], close[i]) == -1
            && open[i] > open[i-1]
            && close[i] < close[i-1]
            && close[i] > close[i-2]) as i32
            * -100;
...
mod tests {
    ...
    fn matches_batch() {
        ...
        let c: Vec<f64> = o
            ...
            .collect();
        let e = crate::stream::candle_upside_gap_two_crows(&o, &h, &l, &c).unwrap();
