use super::{rolling_standard_deviation::RollingStandardDeviation, StreamingIndicator};

#[test]
fn scalar_bulk_and_reset_are_invariant() {
    let input: Vec<f64> = (0..96).map(|i| (i as f64 * 0.13).cos()).collect();
    let mut scalar = RollingStandardDeviation::new(9, 1.0).unwrap();
    let scalar_out: Vec<f64> = input
        .iter()
        .map(|&x| scalar.append(x).unwrap_or(f64::NAN))
        .collect();
    let mut bulk = RollingStandardDeviation::new(9, 1.0).unwrap();
