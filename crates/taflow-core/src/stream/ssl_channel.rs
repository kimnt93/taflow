...
impl SmoothedTrendChannel {
    ...
    pub fn new(period: usize) -> TaResult<Self> {
        ...
        Ok(Self { period, highs: VecDeque::with_capacity(period), lows: VecDeque::with_capacity(period), high_sum: 0.0, low_sum: 0.0, side: 1, value: None })
    }
}
...
impl SmoothedTrendChannel {
    ...
    pub fn append(&mut self, high: f64, low: f64, close: f64) -> Option<(f64, f64)> {
        self.highs.push_back(high); self.high_sum += high;
        self.lows.push_back(low); self.low_sum += low;
        if self.highs.len() > self.period { self.high_sum -= self.highs.pop_front().unwrap(); self.low_sum -= self.lows.pop_front().unwrap(); }
        ...
        if close > average_high { self.side = 1; } else if close < average_low { self.side = -1; }
        self.value = Some(if self.side > 0 { (average_low, average_high) } else { (average_high, average_low) });
        self.value
    }
    ...
    pub fn reset(&mut self) { self.highs.clear(); self.lows.clear(); self.high_sum = 0.0; self.low_sum = 0.0; self.side = 1; self.value = None; }
}
