...
#[derive(Debug, Clone)]
/// Persistent Rust state or aligned output type for `ParabolicMovingAverageStop`.
///
...
impl ParabolicMovingAverageStop {
    ...
    pub fn append(&mut self, high: f64, low: f64, close: f64) -> (f64, i32) {
        ...
        self.upper = Some(match self.upper {
            None => upper,
            Some(previous) if self.trend < 0 => upper.min(previous),
            Some(_) => upper,
        });
        self.lower = Some(match self.lower {
            None => lower,
            Some(previous) if self.trend > 0 => lower.max(previous),
            Some(_) => lower,
        });
        if self.trend > 0 && close < self.lower.unwrap() {
            self.trend = -1;
        } else if self.trend < 0 && close > self.upper.unwrap() {
            self.trend = 1;
        }
        let result = (
            if self.trend > 0 {
    ...
    pub fn value(&self) -> Option<(f64, i32)> {
        self.value
    }
