...
impl CandleInNeck {
    ...
    pub fn extend_slices_into(
        ...
        let scores = candle_in_neck(open, high, low, close)?;
...
pub fn candle_in_neck(
    ...
    for i in start..len {
        ...
        output[i] = (candle_color(open[i-1], close[i-1]) == -1
            && real_body(open[i-1], close[i-1]) > ca_realbody(BODY_LONG, body_sum, open, high, low, close, i-1)
            // 2nd: white, opens below prev low
            && candle_color(open[i], close[i]) == 1
            && open[i] < low[i-1]
            // close slightly into prev body: >= prev close and <= prev close + Equal avg
            && close[i] >= close[i-1]
            && close[i] <= close[i-1] + ca_highlow(EQUAL, equal_sum, open, high, low, close, i-1))
            as i32
            * -100;
...
mod tests {
    ...
    fn matches_batch() {
        ...
        let c: Vec<f64> = o
            ...
            .collect();
        let e = crate::stream::candle_in_neck(&o, &h, &l, &c).unwrap();
