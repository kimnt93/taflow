"""Persistent Ulcer Index."""
from typing import Any
import numpy as np
from ._native import UlcerIndexOperator as _Native
from ._series import as_float64_series
class UlcerIndex:
    """Stateful UlcerIndex indicator.
    Parameters are documented by the constructor signature; scalar
    ``append`` returns the current value and ``compute`` returns
    the aligned history with NaN warm-up where applicable.
    """
    def __init__(self,timeperiod:int=14,_input:Any|None=None):self._state=_Native(timeperiod);self.extend(_input) if _input is not None else None
