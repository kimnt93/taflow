...
from .._native import AutomaticFibonacci as _Native
...
class AutomaticFibonacci:
    ...
    def append(self, high: float, low: float) -> "AutomaticFibonacci":
        """Append one high/low observation and return this adapter."""
        self._state.append(float(high), float(low))
        return self

    def extend(self, high: Any, low: Any) -> "AutomaticFibonacci":
    ...
    @property
    def value(self) -> tuple[float, float, float, float, float, float, float] | None:
    ...
    def compute(
        ...
        """Return the 7 aligned native output histories in documented order."""
        return self._state.compute()

    def reset(self) -> "AutomaticFibonacci":
