"""Public adapter for the native RollingOmegaRatio state."""

from typing import Any

import numpy as np

from .._native import RollingOmegaRatio as _Native
from .._series import as_float64_series


class RollingOmegaRatio:
    """Rolling gain above a threshold divided by loss below it.

    Rust owns the rolling window, arithmetic, warm-up, and aligned output
    history. This rolling series maps to Wickra ``OmegaRatio`` 0.9.9; TA-Lib
    has no direct equivalent.

    Args:
        values: Chronological return or profit/loss series.
        timeperiod: Rolling window length. Defaults to 14.
        threshold: Return threshold separating gains from losses. Defaults to 0.0.

    Raises:
        ValueError: If the window or another configuration value is invalid.
    """

    def __init__(self, timeperiod: int = 14, threshold: float = 0.0) -> None:
        """Initialize an empty configured native state.
        """
        self._state = _Native(int(timeperiod), float(threshold))

    def append(self, value: float) -> "RollingOmegaRatio":
        """Append one observation and return this instance."""
        self._state.append(float(value))
        return self

    def extend(self, values: Any) -> "RollingOmegaRatio":
        """Append a chronological series and return this instance."""
        self._state.extend(as_float64_series(values))
        return self

    @property
    def value(self) -> float | None:
        """Return the latest result, or ``None`` during warm-up."""
        return self._state.value

    def compute(self) -> np.ndarray:
        """Return aligned results, including warm-up ``NaN`` positions."""
        return self._state.compute()

    def reset(self) -> "RollingOmegaRatio":
        """Restore fresh-state behavior and return this instance."""
        self._state.reset()
        return self

    def __len__(self) -> int:
        """Return the number of observations processed by Rust."""
        return len(self._state)


__all__ = ["RollingOmegaRatio"]
