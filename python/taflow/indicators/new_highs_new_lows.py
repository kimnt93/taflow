...
class NewHighsNewLows:
    """Net new-high count minus new-low count for breadth observations."""
    ...
    def append(self,change:float,volume:float,new_high:float,new_low:float)->"NewHighsNewLows":self._state.append(float(change),float(volume),float(new_high),float(new_low));return self
    def extend(self,change:Any,volume:Any,new_high:Any,new_low:Any)->"NewHighsNewLows":
    ...
    def reset(self)->"NewHighsNewLows":self._state.reset();return self
