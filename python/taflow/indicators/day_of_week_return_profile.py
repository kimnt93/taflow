...
class DayOfWeekReturnProfile:
    """Expanding mean close return for the current weekday; Wickra alias ``DayOfWeekProfile``."""
    ...
    def append(self,open:float,high:float,low:float,close:float,volume:float,timestamp:int)->"DayOfWeekReturnProfile":self._state.append(float(open),float(high),float(low),float(close),float(volume),int(timestamp));return self
    def extend(self,open:Any,high:Any,low:Any,close:Any,volume:Any,timestamp:Any)->"DayOfWeekReturnProfile":
    ...
    def reset(self)->"DayOfWeekReturnProfile":self._state.reset();return self
