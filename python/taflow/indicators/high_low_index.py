...
class HighLowIndex:
    """Rolling percentage of new highs among total new highs and lows."""
    ...
    def append(self,change:float,volume:float,new_high:float,new_low:float)->"HighLowIndex":self._state.append(float(change),float(volume),float(new_high),float(new_low));return self
    def extend(self,change:Any,volume:Any,new_high:Any,new_low:Any)->"HighLowIndex":
    ...
    def reset(self)->"HighLowIndex":self._state.reset();return self
