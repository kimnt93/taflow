"""Public adapter for the native RollingProfitFactor state."""
...
class RollingProfitFactor:
    """Measure rolling gross gains divided by absolute gross losses.

    Rust owns the rolling window, arithmetic, warm-up, and aligned output
    history. This rolling series maps to Wickra ``ProfitFactor`` 0.9.9; TA-Lib
    ...
    def append(self, value: float) -> "RollingProfitFactor":
        """Append one observation and return this instance."""
        self._state.append(float(value))
        return self

    def extend(self, values: Any) -> "RollingProfitFactor":
    ...
    def reset(self) -> "RollingProfitFactor":
