...
class OvernightGap:
    """Open-to-prior-session-close return emitted at session boundaries."""
    ...
    def append(self,open:float,high:float,low:float,close:float,volume:float,timestamp:int)->"OvernightGap":self._state.append(float(open),float(high),float(low),float(close),float(volume),int(timestamp));return self
    def extend(self,open:Any,high:Any,low:Any,close:Any,volume:Any,timestamp:Any)->"OvernightGap":
    ...
    def reset(self)->"OvernightGap":self._state.reset();return self
