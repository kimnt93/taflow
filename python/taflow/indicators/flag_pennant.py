...
class FlagPennant:
    """Causal directional impulse followed by narrowing-consolidation signal."""
    def __init__(self,open:Any,high:Any,low:Any,close:Any)->None:self._state=_Native();self.extend(open,high,low,close)
    def append(self,open:float,high:float,low:float,close:float)->"FlagPennant":self._state.append(float(open),float(high),float(low),float(close));return self
    ...
    def reset(self)->"FlagPennant":self._state.reset();return self
