"""Public adapter for the native RollingConditionalValueAtRisk state."""
...
class RollingConditionalValueAtRisk:
    """Estimate mean loss beyond a rolling empirical quantile.

    Rust owns the rolling window, arithmetic, warm-up, and aligned output
    history. This rolling series maps to Wickra ``ConditionalValueAtRisk`` 0.9.9; TA-Lib
    ...
    def append(self, value: float) -> "RollingConditionalValueAtRisk":
        """Append one observation and return this instance."""
        self._state.append(float(value))
        return self

    def extend(self, values: Any) -> "RollingConditionalValueAtRisk":
    ...
    def reset(self) -> "RollingConditionalValueAtRisk":
