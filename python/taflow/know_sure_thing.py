...
class KnowSureThing:
    ...
    """
    def __init__(self, close: Any, roc1: int = 10, roc2: int = 15,
                 roc3: int = 20, roc4: int = 30, sma1: int = 10,
                 sma2: int = 10, sma3: int = 10, sma4: int = 15,
                 signal: int = 9) -> None:
        self._state = _Native(int(roc1), int(roc2), int(roc3), int(roc4), int(sma1), int(sma2), int(sma3), int(sma4), int(signal)); self._length = 0
        self.extend(close)
    ...
    def reset(self) -> "KnowSureThing": self._state.reset(); self._length = 0; return self
