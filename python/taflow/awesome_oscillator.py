...
class AwesomeOscillator:
    """Stateful AwesomeOscillator indicator.
    Parameters are documented by the constructor signature; scalar
    ``append`` returns the current value and ``compute`` returns
    the aligned history with NaN warm-up where applicable.
    """
    def __init__(self,high:Any|None=None,low:Any|None=None,fast:int=5,slow:int=34):self._state=_Native(fast,slow);self.extend(high,low) if high is not None or low is not None else None
