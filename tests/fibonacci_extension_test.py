...
from taflow import FibonacciExtension


def test_fibonacci_extension_lifecycle_and_alignment() -> None:
    length = 4
    high = np.arange(10.0, 10.0 + length)
    low = np.arange(5.0, 5.0 + length)
    indicator = FibonacciExtension(high, low)
    ...
    with pytest.raises(ValueError):
        FibonacciExtension([1.0], [0.0, 1.0])
