...
from taflow import MathDegrees


def test_math_degrees_matches_numpy_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-6.0, 6.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (np.degrees)(values)
        actual = MathDegrees().extend(values)
