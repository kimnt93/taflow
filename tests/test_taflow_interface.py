...
def test_descriptive_hilbert_transform_trendline():
    _, _, close = price_data(700)
    indicator = taflow.HilbertTransformTrendline()
    assert_outputs_equal(
        indicator.extend(close),
        original_talib.HT_TRENDLINE(close),
        rtol=1e-10,
        atol=1e-12,
    )
...
def test_descriptive_average_directional_index_rating():
    high, low, close = price_data(700)
    indicator = taflow.AverageDirectionalIndexRating(period=14)
    assert_outputs_equal(
        indicator.extend(high, low, close),
        original_talib.ADXR(high, low, close, 14),
        rtol=1e-10,
        atol=1e-12,
    )
