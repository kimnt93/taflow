...
from taflow import MathAcos


def test_math_acos_matches_talib_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-0.95, 0.95, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (talib.ACOS)(values)
        actual = MathAcos().extend(values)
