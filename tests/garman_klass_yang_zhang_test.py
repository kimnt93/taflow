import numpy as np
from taflow import GarmanKlassYangZhang


def test_garman_klass_yang_zhang_lifecycle():
    open_ = np.linspace(100.0, 120.0, 80)
    high = open_ + 1.0
    low = open_ - 1.0
    close = open_ + 0.5
    indicator = GarmanKlassYangZhang(timeperiod=10).extend(open_, high, low, close)
