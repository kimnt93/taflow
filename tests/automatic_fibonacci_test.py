...
from taflow import AutomaticFibonacci


def test_automatic_fibonacci_lifecycle_and_alignment() -> None:
    length = 4
    high = np.arange(10.0, 10.0 + length)
    low = np.arange(5.0, 5.0 + length)
    indicator = AutomaticFibonacci(high, low)
    ...
    with pytest.raises(ValueError):
        AutomaticFibonacci([1.0], [0.0, 1.0])
