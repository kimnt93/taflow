...
def test_williams_percent_r_matches_talib_matrix_and_lifecycle(timeperiod: int) -> None:
    ...
    actual = WilliamsPercentR(timeperiod).extend(high, low, close)
...
def test_williams_percent_r_handles_flat_short_and_invalid_inputs() -> None:
    ...
    np.testing.assert_array_equal(WilliamsPercentR(4).extend(flat, flat, flat).compute(), expected)
    short = WilliamsPercentR(4).extend(flat[:2], flat[:2], flat[:2])
