...
def test_matches_talib_tema() -> None:
    ...
    actual = TripleExponentialMovingAverage(7).extend(values).compute()
...
def test_lifecycle_is_chunk_invariant() -> None:
    ...
    expected = TripleExponentialMovingAverage(7).extend(values).compute()
    state = TripleExponentialMovingAverage(7)
