...
from taflow import MathTan


def test_math_tan_matches_talib_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-1.0, 1.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (talib.TAN)(values)
        actual = MathTan().extend(values)
