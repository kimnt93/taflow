...
def test_matches_talib_t3() -> None:
    ...
    actual = TripleExponentialAverage(7, 0.7).extend(values).compute()
...
def test_lifecycle_is_chunk_invariant() -> None:
    ...
    expected = TripleExponentialAverage(7, 0.7).extend(values).compute()
    state = TripleExponentialAverage(7, 0.7)
