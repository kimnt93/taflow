...
from taflow import FibonacciTimeZones


def test_fibonacci_time_zones_lifecycle_and_alignment() -> None:
    length = 4
    high = np.arange(10.0, 10.0 + length)
    low = np.arange(5.0, 5.0 + length)
    indicator = FibonacciTimeZones(high, low)
    ...
    with pytest.raises(ValueError):
        FibonacciTimeZones([1.0], [0.0, 1.0])
