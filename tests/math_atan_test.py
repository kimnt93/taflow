...
from taflow import MathAtan


def test_math_atan_matches_talib_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-8.0, 8.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (talib.ATAN)(values)
        actual = MathAtan().extend(values)
