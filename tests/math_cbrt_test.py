...
from taflow import MathCbrt


def test_math_cbrt_matches_numpy_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-64.0, 64.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (np.cbrt)(values)
        actual = MathCbrt().extend(values)
