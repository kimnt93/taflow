...
from taflow import MathRadians


def test_math_radians_matches_numpy_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(-720.0, 720.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (np.radians)(values)
        actual = MathRadians().extend(values)
