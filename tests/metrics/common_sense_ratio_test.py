...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([-0.10, -0.04, -0.01, 0.0, 0.02, 0.05, 0.12]),
        np.array([-0.20]),
        np.linspace(-0.10, 0.08, 101),
        np.array([-0.05, -0.05, -0.01, 0.02, 0.02, 0.07]),
        np.array([np.nan, -0.08, 0.01, np.nan, -0.02, 0.03]),
        np.random.default_rng(20260811).normal(0.0004, 0.012, 513),
    ],
)
def test_common_sense_ratio_matches_quantstats_components(
    returns: np.ndarray,
) -> None:
...
def test_common_sense_ratio_matches_direct_quantstats_on_clean_returns() -> None:
    returns = pd.Series([-0.10, -0.04, -0.01, 0.0, 0.02, 0.05, 0.12])
...
def test_common_sense_ratio_lifecycle_and_missing_are_invariant() -> None:
    returns = np.array([-0.10, np.nan, -0.04, -0.01, 0.0, 0.02, 0.05, 0.12])
...
def test_common_sense_ratio_undefined_and_invalid_edges() -> None:
    ...
    with pytest.raises(ValueError):
        CommonSenseRatio.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_common_sense_ratio_column_selection() -> None:
    frame = pd.DataFrame(
        {"strategy": [-0.1, 0.02, 0.03], "other": [0.5, 0.6, 0.7]}
    )
    expected = CommonSenseRatio.from_returns(frame["strategy"]).compute()
    assert CommonSenseRatio.from_returns(
        frame, column="strategy"
