...
def _vectorbt_probabilistic_sharpe(
    returns: np.ndarray,
...
@pytest.mark.parametrize(
    ("returns", "periods_per_year", "annual_risk_free_rate", "benchmark"),
    [
        (np.array([0.02, -0.01, 0.03, -0.025, 0.01, -0.04, 0.015]), 252.0, 0.0, 0.5),
        (np.linspace(-0.04, 0.05, 101), 52.0, -0.01, -0.2),
        (np.array([0.15, -0.1, -0.1, 0.25, -0.3, 0.1]), 12.0, 0.04, 1.2),
        (np.array([np.nan, 0.02, -0.03, np.nan, 0.01, 0.04]), 252.0, 0.03, 0.0),
        (np.random.default_rng(20260812).standard_t(5.0, 513) * 0.012, 365.0, 0.05, 0.8),
    ],
)
def test_probabilistic_sharpe_matches_pinned_vectorbt_formula(
    returns: np.ndarray,
    periods_per_year: float,
    annual_risk_free_rate: float,
    benchmark: float,
    ...
    actual = ProbabilisticSharpeRatio.from_returns(
        returns,
        periods_per_year=periods_per_year,
        annual_risk_free_rate=annual_risk_free_rate,
        annual_benchmark_sharpe_ratio=benchmark,
    ).compute()
    expected = _vectorbt_probabilistic_sharpe(
        returns, periods_per_year, annual_risk_free_rate, benchmark
    )
    if expected is None:
...
def test_probabilistic_sharpe_factories_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.10, -0.20, 0.05, -0.03, 0.08, -0.01])
    settings = {
        "periods_per_year": 12.0,
        "annual_risk_free_rate": 0.03,
        "annual_benchmark_sharpe_ratio": 0.4,
    }
    expected = ProbabilisticSharpeRatio.from_returns(returns, **settings).compute()
    equity = 100.0 * np.r_[1.0, np.cumprod(1.0 + returns)]
    pnl = np.diff(equity)
    assert ProbabilisticSharpeRatio.from_equity(equity, **settings).compute() == pytest.approx(expected)
    assert ProbabilisticSharpeRatio.from_pnl(
        pnl, initial_equity=100.0, **settings
    ).compute() == pytest.approx(expected)
    assert ProbabilisticSharpeRatio.from_log_returns(
        np.log1p(returns), **settings
    ).compute() == pytest.approx(expected)

    state = ProbabilisticSharpeRatio.from_returns([], **settings)
...
def test_probabilistic_sharpe_edges_and_validation() -> None:
    ...
    with pytest.raises(ValueError):
        ProbabilisticSharpeRatio.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
        ProbabilisticSharpeRatio.from_returns([np.inf])
    with pytest.raises(ValueError):
        ProbabilisticSharpeRatio.from_returns([-1.01])
    with pytest.raises(ValueError):
        ProbabilisticSharpeRatio.from_returns([0.01] * 4, periods_per_year=0.0)
    with pytest.raises(ValueError):
        ProbabilisticSharpeRatio.from_returns(
            [0.01] * 4, annual_benchmark_sharpe_ratio=np.inf
        )


def test_probabilistic_sharpe_requires_semantic_factory() -> None:
