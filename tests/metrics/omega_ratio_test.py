...
def _empyrical_omega(
    ...
    expected = float(
        empyrical.omega_ratio(
            usable,
            required_return=annual_required_return,
...
def test_omega_ratio_matches_empyrical(
    returns: np.ndarray,
    periods_per_year: float,
    annual_required_return: float,
) -> None:
    actual = OmegaRatio.from_returns(
        returns,
        periods_per_year=periods_per_year,
        annual_required_return=annual_required_return,
    ).compute()
    expected = _empyrical_omega(
        returns, periods_per_year, annual_required_return
    )
...
def test_omega_ratio_factories_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.10, -0.20, 0.05])
    settings = {"periods_per_year": 12.0, "annual_required_return": 0.04}
    expected = OmegaRatio.from_returns(returns, **settings).compute()
    equity = np.array([100.0, 110.0, 88.0, 92.4])
    pnl = np.array([10.0, -22.0, 4.4])

    assert OmegaRatio.from_equity(equity, **settings).compute() == pytest.approx(
        expected
    )
    assert OmegaRatio.from_pnl(
        pnl, initial_equity=100.0, **settings
    ).compute() == pytest.approx(expected)
    assert OmegaRatio.from_log_returns(
        np.log1p(returns), **settings
    ).compute() == pytest.approx(expected)

    state = OmegaRatio.from_returns([], **settings)
...
def test_omega_ratio_missing_boundaries_and_invalid_values() -> None:
    ...
    assert OmegaRatio.from_returns([0.25, 0.10]).compute() is None
    assert OmegaRatio.from_returns([-0.25, -0.10]).compute() == 0.0
    assert len(OmegaRatio.from_returns([np.nan, 0.01, -0.02])) == 2
    with pytest.raises(ValueError):
        OmegaRatio.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
        OmegaRatio.from_returns([np.inf])
    with pytest.raises(ValueError):
        OmegaRatio.from_returns([0.01, -0.02], periods_per_year=0.0)
    with pytest.raises(ValueError):
        OmegaRatio.from_returns(
            [0.01, -0.02], annual_required_return=-1.0
        )


def test_omega_ratio_requires_semantic_factory() -> None:
