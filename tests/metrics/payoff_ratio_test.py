...
def test_payoff_ratio_raw_pnl_and_trade_factories_preserve_values() -> None:
    observations = np.array([100.0, -40.0, 0.0, 20.0, -10.0])
    expected = 60.0 / 25.0
    assert PayoffRatio.from_pnl(observations).compute() == pytest.approx(expected)
    assert PayoffRatio.from_trades(observations).compute() == pytest.approx(
        expected
    )
...
def test_payoff_ratio_boundaries_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        PayoffRatio.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_payoff_ratio_requires_semantic_factory() -> None:
