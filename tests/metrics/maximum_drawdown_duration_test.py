...
def test_input_methods_and_lifecycle():
    ...
    assert MaximumDrawdownDuration().from_pnl(np.diff(equity), initial_capital=100).compute() == expected
...
def test_validation():
    unbound = MaximumDrawdownDuration()
    with pytest.raises(ValueError):
        unbound.append(0.01)
    assert MaximumDrawdownDuration().from_returns([0.1, 0.2]).compute() is None
    with pytest.raises(ValueError):
        MaximumDrawdownDuration().from_returns([np.inf])
