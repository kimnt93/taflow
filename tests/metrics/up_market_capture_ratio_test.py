...
def _empyrical_up_capture(
    ...
    if periods_per_year == 252.0:
        ...
        primary_cagr = float(
            empyrical.annual_return(primary, annualization=periods_per_year)
        )
        benchmark_cagr = float(
            empyrical.annual_return(benchmark, annualization=periods_per_year)
        )
...
def test_up_market_capture_ratio_matches_empyrical(
...
def test_up_market_capture_ratio_uses_explicit_annualization() -> None:
    ...
    actual = UpMarketCaptureRatio.from_returns(
...
def test_up_market_capture_ratio_factories_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.10, -0.20, 0.05])
    benchmark_returns = np.array([0.02, -0.10, 0.01])
    expected = UpMarketCaptureRatio.from_returns(
    ...
    assert UpMarketCaptureRatio.from_equity(
        equity, benchmark_equity, periods_per_year=12.0
    ).compute() == pytest.approx(expected)
    assert UpMarketCaptureRatio.from_pnl(
    ...
    assert UpMarketCaptureRatio.from_log_returns(
        np.log1p(returns),
        np.log1p(benchmark_returns),
        periods_per_year=12.0,
    ).compute() == pytest.approx(expected)

    state = UpMarketCaptureRatio.from_returns([], [], periods_per_year=12.0)
...
def test_up_market_capture_ratio_pairwise_missing_filter_and_edges() -> None:
    ...
    assert actual.compute() == pytest.approx(
        _empyrical_up_capture(returns, benchmark)
    )
    assert UpMarketCaptureRatio.from_returns([0.25], [0.10]).compute() is not None
    assert UpMarketCaptureRatio.from_returns(
        [0.01, 0.02], [0.0, -0.01]
    ).compute() is None
    smallest_positive = float(np.nextafter(0.0, 1.0))
    assert UpMarketCaptureRatio.from_returns(
        [0.01], [smallest_positive], periods_per_year=smallest_positive
    ).compute() is None
...
def test_up_market_capture_ratio_missing_invalid_and_factory_contracts() -> None:
    with pytest.raises(ValueError):
        UpMarketCaptureRatio.from_returns([np.nan], [0.01], nan_policy="raise")
    with pytest.raises(ValueError):
        UpMarketCaptureRatio.from_returns([np.inf], [0.01])
    with pytest.raises(ValueError):
        UpMarketCaptureRatio.from_returns([0.01, 0.02], [0.01])
    with pytest.raises(ValueError):
        UpMarketCaptureRatio.from_returns([0.01], [0.02], periods_per_year=0.0)
