...
@pytest.mark.parametrize("values", [
    np.array([0.01, -0.02, 0.03, 0.015, -0.004]),
    np.linspace(-0.02, 0.03, 30),
    np.array([0.01, np.nan, -0.005, 0.02]),
])
...
def test_factories_and_lifecycle_are_equivalent():
    ...
    assert StabilityOfTimeSeries.from_pnl(pnl, initial_equity=100.0).compute() == pytest.approx(expected)
...
def test_validation_and_warmup():
    ...
    with pytest.raises(ValueError):
        StabilityOfTimeSeries.from_returns([np.nan], nan_policy="raise")
