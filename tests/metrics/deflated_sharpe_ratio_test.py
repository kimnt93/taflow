...
@pytest.mark.parametrize(('returns', 'ppy', 'rf', 'trials', 'variance'), [(np.array([0.02, -0.01, 0.03, -0.025, 0.01, -0.04, 0.015]), 252.0, 0.0, 20, 0.64), (np.linspace(-0.04, 0.05, 101), 52.0, -0.01, 5, 0.2), (np.random.default_rng(91).standard_t(5, 513) * 0.012, 365.0, 0.05, 100, 1.4), (np.array([np.nan, 0.02, -0.03, np.nan, 0.01, 0.04]), 12.0, 0.03, 2, 0.0)])
def test_deflated_sharpe_matches_vectorbt(returns: np.ndarray, ppy: float, rf: float, trials: int, variance: float) -> None:
    ...
    actual = DeflatedSharpeRatio(number_of_trials=trials, annual_sharpe_ratio_variance=variance, periods_per_year=ppy, annual_risk_free_rate=rf).from_returns(returns).compute()
    expected = _vectorbt_dsr(returns, ppy, rf, trials, variance)
    assert actual == pytest.approx(expected, rel=2e-09, abs=2e-10)

def test_deflated_sharpe_input_methods_and_lifecycle() -> None:
    ...
    expected = DeflatedSharpeRatio(**settings).from_returns(returns).compute()
    equity = 100 * np.r_[1.0, np.cumprod(1 + returns)]
    assert DeflatedSharpeRatio(**settings).from_equity(equity).compute() == pytest.approx(expected)
    assert DeflatedSharpeRatio(**settings).from_pnl(np.diff(equity), initial_capital=100).compute() == pytest.approx(expected)
    assert DeflatedSharpeRatio(**settings).from_log_returns(np.log1p(returns)).compute() == pytest.approx(expected)
    state = DeflatedSharpeRatio(**settings).from_returns([])
...
def test_deflated_sharpe_edges_and_validation() -> None:
    ...
    assert DeflatedSharpeRatio(**kwargs).from_returns([0.01, -0.01, 0.02]).compute() is None
    assert DeflatedSharpeRatio(**kwargs).from_returns([0.01] * 8).compute() is None
    with pytest.raises(ValueError):
        DeflatedSharpeRatio(number_of_trials=1, annual_sharpe_ratio_variance=0.1).from_returns([0.01] * 4)
    with pytest.raises(ValueError):
        DeflatedSharpeRatio(number_of_trials=2, annual_sharpe_ratio_variance=-0.1).from_returns([0.01] * 4)
    unbound = DeflatedSharpeRatio(**kwargs)
    with pytest.raises(ValueError):
        unbound.append(0.01)
