...
@pytest.mark.parametrize(
    "trades",
    [
        np.array([100.0, -40.0, 20.0, -10.0, 80.0]),
        np.array([-100.0, 20.0, -50.0, 10.0]),
        np.array([-10.0, 10.0]),
        np.array([10.0]),
        np.full(32, 25.0),
        np.linspace(-400.0, 500.0, 101),
        np.array([np.nan, 20.0, -30.0, np.nan, 10.0]),
        np.random.default_rng(20260811).normal(40.0, 1200.0, 513),
    ],
)
def test_system_quality_number_matches_numpy_and_pinned_vectorbt_formula(
    trades: np.ndarray,
) -> None:
    # vectorbt 0.28.1 Trades.sqn uses count, pnl.mean, and pnl.std whose default
    # ddof is one. This is source parity plus an executable NumPy cross-check.
...
def test_system_quality_number_boundaries_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        SystemQualityNumber.from_trades([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
        SystemQualityNumber.from_trades([np.inf])


def test_system_quality_number_column_selection_and_factory_contract() -> None:
    frame = pd.DataFrame({"pnl": [100.0, -50.0], "other": [1.0, 2.0]})
    expected = _numpy_system_quality_number(frame["pnl"].to_numpy())
    assert SystemQualityNumber.from_trades(
        frame, column="pnl"
