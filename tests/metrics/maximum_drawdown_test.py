...
def test_maximum_drawdown_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert MaximumDrawdown.from_pnl(
        pnl, initial_equity=100.0
    ).compute() == pytest.approx(expected)
    assert MaximumDrawdown.from_log_returns(np.log1p(returns)).compute() == pytest.approx(
        expected
    )
...
def test_maximum_drawdown_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        MaximumDrawdown.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_maximum_drawdown_requires_semantic_factory() -> None:
