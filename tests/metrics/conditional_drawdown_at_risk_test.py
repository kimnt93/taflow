...
@pytest.mark.parametrize(
    ...
    [
        ...
        np.array([0.20]),
        np.zeros(32),
        np.array([0.25, -0.20, 0.25, 0.10, -0.50, 1.0, -0.10]),
        np.array([0.25, -0.20, 0.25, 0.25, -0.20]),
        np.linspace(-0.04, 0.05, 101),
        np.array([np.nan, 0.02, -0.03, np.nan, 0.01]),
        np.random.default_rng(20260812).normal(0.0004, 0.012, 513),
    ],
)
def test_matches_pinned_performanceanalytics_discrete_source(
    returns: np.ndarray, confidence: float
) -> None:
    # Source-specification parity is pinned separately from the Riskfolio
    # continuous-path variant; the aggregate verifier can run the R oracle.
    assert len(PERFORMANCE_ANALYTICS_2_1_0_SHA256) == 64
    assert len(RISKFOLIO_LIB_7_3_0_SHA256) == 64
    actual = ConditionalDrawdownAtRisk.from_returns(
...
def test_factories_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.25, -0.20, 0.25, 0.10, -0.50, 1.0, -0.10])
    expected = ConditionalDrawdownAtRisk.from_returns(
        returns, confidence=0.75
    ).compute()
    equity = 100.0 * np.r_[1.0, np.cumprod(1.0 + returns)]
    pnl = np.diff(equity)

    assert ConditionalDrawdownAtRisk.from_equity(
        equity, confidence=0.75
    ).compute() == pytest.approx(expected)
    assert ConditionalDrawdownAtRisk.from_pnl(
        pnl, initial_equity=100.0, confidence=0.75
    ).compute() == pytest.approx(expected)
    assert ConditionalDrawdownAtRisk.from_log_returns(
        np.log1p(returns), confidence=0.75
    ).compute() == pytest.approx(expected)

    state = ConditionalDrawdownAtRisk.from_returns([], confidence=0.75)
...
def test_current_episode_zero_paths_and_boundary_ties() -> None:
    state = ConditionalDrawdownAtRisk.from_returns([0.25, -0.20])
    assert state.compute() == pytest.approx(0.20)
    assert state.append(0.25).compute() == pytest.approx(0.20)
    assert ConditionalDrawdownAtRisk.from_returns([0.10, 0.0, 0.20]).compute() == 0.0
    assert ConditionalDrawdownAtRisk.from_returns(
        [0.25, -0.20, 0.25, 0.25, -0.20]
    ).compute() == pytest.approx(0.20)


def test_missing_and_invalid_values_are_enforced() -> None:
    returns = np.array([0.25, np.nan, -0.20, 0.25, -0.10])
    usable = returns[~np.isnan(returns)]
    state = ConditionalDrawdownAtRisk.from_returns(returns)
    assert len(state) == len(usable)
    assert state.compute() == pytest.approx(
        _performanceanalytics_discrete(usable, 0.95)
    )
    for confidence in [0.0, 1.0, -0.1, 1.1, np.nan, np.inf]:
        with pytest.raises(ValueError):
            ConditionalDrawdownAtRisk.from_returns([0.01], confidence=confidence)
    with pytest.raises(ValueError):
        ConditionalDrawdownAtRisk.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_requires_semantic_factory() -> None:
