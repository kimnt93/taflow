...
def quantstats_ulcer_performance_index(returns: np.ndarray) -> float:
    ...
    series = pd.Series(
        filtered,
        index=pd.date_range("2000-01-01", periods=len(filtered), freq="D"),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        np.array([0.15, -0.10, -0.10, 0.25, -0.30, 0.10]),
        np.array([0.20, -0.20, 0.25, -0.20, 0.25, -0.20]),
...
def test_ulcer_performance_index_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert UlcerPerformanceIndex.from_equity(equity).compute() == pytest.approx(
        expected
    )
    assert UlcerPerformanceIndex.from_pnl(
        pnl, initial_equity=100.0
    ).compute() == pytest.approx(expected)
    assert UlcerPerformanceIndex.from_log_returns(
        np.log1p(returns)
    ).compute() == pytest.approx(expected)
...
def test_ulcer_performance_index_warmup_zero_risk_and_invalid_contract() -> None:
    ...
    assert UlcerPerformanceIndex.from_returns([-0.10]).compute() is None
    assert UlcerPerformanceIndex.from_returns([0.10, 0.0, 0.20]).compute() is None
    assert len(UlcerPerformanceIndex.from_returns([np.nan, -0.01, -0.01])) == 2

    negative = UlcerPerformanceIndex.from_returns([-0.10, -0.10]).compute()
    assert negative is not None and negative < 0.0

    with pytest.raises(ValueError, match="NaN"):
        UlcerPerformanceIndex.from_returns([np.nan], nan_policy="raise")
    ...
    with pytest.raises(ValueError, match="initial_equity"):
        UlcerPerformanceIndex.from_pnl([1.0], initial_equity=0.0)


def test_ulcer_performance_index_requires_semantic_factory() -> None:
