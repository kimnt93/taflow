...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([0.10, -0.04, 0.0, 0.02, -0.01]),
        np.array([-0.20]),
...
def test_profit_factor_raw_pnl_trade_factories_and_lifecycle() -> None:
...
def test_profit_factor_zero_denominator_normalization() -> None:
    ...
    # QuantStats returns zero for all-zero input; TAFlow's documented undefined
    # normalization distinguishes the 0/0 case from a loss-only factor of zero.
...
def test_profit_factor_missing_invalid_and_domain_semantics() -> None:
    ...
    with pytest.raises(ValueError):
        ProfitFactor.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
        ProfitFactor.from_returns([np.inf])
    with pytest.raises(ValueError):
        ProfitFactor.from_returns([-2.0])

    # Raw monetary observations are not constrained to the simple-return floor.
    assert ProfitFactor.from_pnl([-2.0]).compute() == 0.0
    assert ProfitFactor.from_trades([-2.0]).compute() == 0.0


def test_profit_factor_column_selection_and_factory_contract() -> None:
    frame = pd.DataFrame({"strategy": [0.1, -0.05], "other": [9.0, 8.0]})
    assert ProfitFactor.from_returns(frame, column="strategy").compute() == pytest.approx(
        2.0
    )
    with pytest.raises(ValueError):
