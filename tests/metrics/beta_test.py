...
def test_beta_matches_empyrical(
    returns: np.ndarray, benchmark_returns: np.ndarray
) -> None:
...
def test_beta_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert Beta.from_equity(equity, benchmark_equity).compute() == pytest.approx(
        expected
    )
    assert Beta.from_pnl(
        np.array([10.0, -22.0, 4.4]),
        np.array([4.0, -20.4, 1.836]),
        initial_equity=100.0,
        benchmark_initial_equity=200.0,
    ).compute() == pytest.approx(expected)
    assert Beta.from_log_returns(
...
def test_beta_pairwise_missing_minimum_and_zero_variance() -> None:
    ...
    assert Beta.from_returns([0.25], [0.10]).compute() is None
    assert Beta.from_returns([0.01, 0.02], [0.10, 0.10]).compute() is None


def test_beta_rejects_misalignment_without_mutation() -> None:
    state = Beta.from_returns([0.01, 0.02], [0.00, 0.01])
...
def test_beta_missing_invalid_and_factory_contracts() -> None:
    with pytest.raises(ValueError):
        Beta.from_returns([np.nan], [0.0], nan_policy="raise")
    with pytest.raises(ValueError):
