    ...
    for drawdown in drawdowns:
        if drawdown < 0.0:
            current_depth = (
                ...
                else min(current_depth, float(drawdown))
            )
...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([0.25, -0.20, 0.0, 0.10, -0.50, 1.0, -0.10]),
        np.array([-0.20]),
        np.array([0.20]),
        np.zeros(32),
...
def test_average_drawdown_matches_pinned_performanceanalytics_source_convention(
    returns: np.ndarray,
) -> None:
...
def test_average_drawdown_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert AverageDrawdown.from_pnl(
        pnl, initial_equity=100.0
    ).compute() == pytest.approx(expected)
    assert AverageDrawdown.from_log_returns(np.log1p(returns)).compute() == pytest.approx(
        expected
    )
...
def test_average_drawdown_episode_boundaries_and_current_episode() -> None:
    state = AverageDrawdown.from_returns([0.25, -0.20])
    assert state.compute() == pytest.approx(0.20)
    assert state.append(0.25).compute() == pytest.approx(0.20)
    assert state.extend([0.10, -0.50]).compute() == pytest.approx(0.35)
    assert AverageDrawdown.from_returns([0.10, 0.0, 0.20]).compute() == 0.0


def test_average_drawdown_missing_and_invalid_values() -> None:
    filtered = np.array([0.25, -0.20, 0.25, -0.10])
    state = AverageDrawdown.from_returns([0.25, np.nan, -0.20, 0.25, -0.10])
    assert len(state) == len(filtered)
    assert state.compute() == pytest.approx(
        _performance_analytics_source_convention(filtered)
    )
    with pytest.raises(ValueError):
        AverageDrawdown.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_average_drawdown_requires_semantic_factory() -> None:
