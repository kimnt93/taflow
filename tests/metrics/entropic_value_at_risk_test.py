...
RISK_FUNCTIONS_SHA256 = (
    "eed72dceb7024c9ead811fa12d8e834618604f8db9c4fbb6367ce4d2a3992719"
)
...
def _riskfolio_empirical_evar(returns: np.ndarray, cutoff: float) -> float:
    ...
    result = optimize.minimize_scalar(
        objective,
        bounds=(np.log(scale) - 40.0, np.log(scale) + 40.0),
        method="bounded",
        options={"xatol": 1e-13, "maxiter": 1_000},
    )
...
@pytest.mark.parametrize(
    ("returns", "cutoff"),
    [
        (np.linspace(-0.04, 0.05, 101), 0.05),
        (np.sin(np.arange(257) * 0.37) * 0.03, 0.10),
        (np.random.default_rng(20260812).normal(0.0004, 0.012, 513), 0.025),
        (np.r_[np.nan, np.linspace(-0.03, 0.025, 128), np.nan], 0.20),
        (np.repeat(0.0125, 64), 0.05),
        (np.array([0.02, -0.01, -0.04, 0.03]), 0.05),
    ],
)
def test_entropic_value_at_risk_matches_riskfolio_formula(
    ...
    actual = EntropicValueAtRisk.from_returns(returns, cutoff=cutoff).compute()
    assert actual == pytest.approx(expected, rel=2e-10, abs=2e-12)


def test_entropic_value_at_risk_factories_and_lifecycle_are_invariant() -> None:
    returns = np.sin(np.arange(64) * 0.71) * 0.025
    expected = EntropicValueAtRisk.from_returns(returns, cutoff=0.10).compute()
    equity = 100.0 * np.r_[1.0, np.cumprod(1.0 + returns)]
    pnl = np.diff(equity)

    assert EntropicValueAtRisk.from_equity(
        equity, cutoff=0.10
    ).compute() == pytest.approx(expected)
    assert EntropicValueAtRisk.from_pnl(
        pnl, initial_equity=100.0, cutoff=0.10
    ).compute() == pytest.approx(expected)
    assert EntropicValueAtRisk.from_log_returns(
        np.log1p(returns), cutoff=0.10
    ).compute() == pytest.approx(expected)

    state = EntropicValueAtRisk.from_returns([], cutoff=0.10)
...
def test_entropic_value_at_risk_edges_and_validation() -> None:
    ...
    assert EntropicValueAtRisk.from_returns([0.0125] * 64).compute() == pytest.approx(
        -0.0125
    )
    assert EntropicValueAtRisk.from_returns([0.02, -0.04], cutoff=0.05).compute() == pytest.approx(
        0.04
    )
    assert len(EntropicValueAtRisk.from_returns([np.nan, 0.02, -0.01])) == 2
    with pytest.raises(ValueError):
        EntropicValueAtRisk.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
    ...
    for cutoff in [0.0, 1.0, np.nan, np.inf]:
        with pytest.raises(ValueError):
            EntropicValueAtRisk.from_returns([0.01], cutoff=cutoff)


def test_entropic_value_at_risk_requires_semantic_factory() -> None:
