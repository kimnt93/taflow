...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([0.02, -0.01, 0.0, 0.03, -0.025, -0.01]),
        np.array([-0.10]),
        np.zeros(32),
...
def test_average_loss_raw_pnl_and_trade_factories_match_quantstats() -> None:
...
def test_average_loss_boundaries_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        AverageLoss.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
        AverageLoss.from_returns([np.inf])
    with pytest.raises(ValueError):
        AverageLoss.from_returns([-1.01])

    # Raw P&L and trades are not constrained by the simple-return lower bound.
    assert AverageLoss.from_pnl([-1000.0, 50.0]).compute() == -1000.0
    assert AverageLoss.from_trades([-1000.0, 50.0]).compute() == -1000.0


def test_average_loss_pnl_factory_rejects_capital_conversion_argument() -> None:
    with pytest.raises(TypeError):
        AverageLoss.from_pnl([1.0], initial_equity=100.0)


def test_average_loss_requires_semantic_factory() -> None:
    with pytest.raises(TypeError):
        AverageLoss()
