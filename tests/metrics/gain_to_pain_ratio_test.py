...
@pytest.mark.parametrize('returns', [np.array([0.1, -0.2]), np.linspace(-0.04, 0.05, 101), np.array([0.15, -0.1, -0.1, 0.25, -0.3, 0.1]), np.array([0.2, -0.2, 0.25, -0.2, 0.25, -0.2]), np.array([np.nan, 0.02, -0.03, np.nan, 0.01]), np.random.default_rng(20260811).normal(0.0004, 0.012, 513)])
...
def test_gain_to_pain_ratio_freezes_net_numerator_and_input_resolution() -> None:
    assert GainToPainRatio().from_returns([0.02, -0.01]).compute() == pytest.approx(1.0)
    returns = np.array([0.03, -0.01, 0.02, -0.01])
    assert GainToPainRatio().from_returns(returns).compute() == pytest.approx(quantstats_gain_to_pain_ratio(returns))

def test_gain_to_pain_ratio_input_methods_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.1, -0.2, 0.05, -0.025, 0.1])
    ...
    assert GainToPainRatio().from_pnl(pnl, initial_capital=100.0).compute() == pytest.approx(expected)
    assert GainToPainRatio().from_log_returns(np.log1p(returns)).compute() == pytest.approx(expected)
...
def test_gain_to_pain_ratio_boundaries_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        GainToPainRatio(nan_policy='raise').from_returns([np.nan])
    with pytest.raises(ValueError):
        GainToPainRatio().from_returns([np.inf])
    with pytest.raises(ValueError):
        GainToPainRatio().from_returns([-1.01])

def test_gain_to_pain_ratio_requires_semantic_ingestion_before_append() -> None:
    with pytest.raises(ValueError):
        GainToPainRatio().append(0.01)
