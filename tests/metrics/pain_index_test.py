...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([0.02, -0.01, 0.015, -0.03, 0.01]),
        np.array([-0.20]),
        np.array([0.20]),
        np.zeros(32),
...
def test_pain_index_matches_pinned_performanceanalytics_source_convention(
    returns: np.ndarray,
) -> None:
...
def test_pain_index_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert PainIndex.from_pnl(pnl, initial_equity=100.0).compute() == pytest.approx(
        expected
    )
    assert PainIndex.from_log_returns(np.log1p(returns)).compute() == pytest.approx(
        expected
    )
...
def test_pain_index_uses_phantom_wealth_but_excludes_it_from_divisor() -> None:
    ...
    # Drawdowns are 0, 0.2, 0; only the three real observations are averaged.
    assert PainIndex.from_returns([0.1, -0.2, 0.25]).compute() == pytest.approx(
        0.2 / 3.0
    )
...
def test_pain_index_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        PainIndex.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_pain_index_requires_semantic_factory() -> None:
