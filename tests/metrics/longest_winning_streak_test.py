...
def _quantstats_consecutive_wins(values: np.ndarray) -> float | None:
    ...
    if usable.size == 0:
        return None
    return float(qs.consecutive_wins(pd.Series(usable), aggregate=None, compounded=True, prepare_returns=False))

@pytest.mark.parametrize('returns', [np.array([0.01, 0.02, 0.0, 0.03, 0.04, 0.05, -0.01, 0.06]), np.array([-0.2]), np.array([0.2]), np.zeros(32), np.linspace(-0.04, 0.05, 101), np.array([0.02, 0.02, -0.01, -0.01, 0.0, 0.02]), np.array([np.nan, 0.02, 0.03, np.nan, 0.01, -0.01]), np.random.default_rng(20260811).normal(0.0004, 0.012, 513)])
...
def test_longest_winning_streak_zero_and_negative_break_runs() -> None:
    ...
    assert LongestWinningStreak().from_returns(values).compute() == _quantstats_consecutive_wins(np.asarray(values))

def test_longest_winning_streak_raw_input_methods_and_lifecycle() -> None:
...
def test_longest_winning_streak_missing_and_invalid_semantics() -> None:
    ...
    with pytest.raises(ValueError):
        LongestWinningStreak(nan_policy='raise').from_returns([np.nan])
    with pytest.raises(ValueError):
...
def test_longest_winning_streak_column_and_input_method_contract() -> None:
    frame = pd.DataFrame({'strategy': [0.1, 0.2], 'other': [9.0, 8.0]})
    assert LongestWinningStreak().from_returns(frame, column='strategy').compute() == 2.0
    with pytest.raises(ValueError):
        LongestWinningStreak().from_returns(frame)
    unbound = LongestWinningStreak()
    with pytest.raises(ValueError):
        unbound.append(0.01)
