...
def test_validation_and_warmup():
    with pytest.raises(TypeError): BreakevenRate()
    assert BreakevenRate.from_returns([]).compute() is None
    with pytest.raises(ValueError): BreakevenRate.from_returns([np.inf])
    with pytest.raises(ValueError): BreakevenRate.from_returns([np.nan], nan_policy="raise")
