...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            np.array([np.nan, 0.02, -0.03, 0.01, np.nan]),
            np.array([-0.01, np.nan, -0.02, 0.005, np.nan]),
        ),
        (
            np.linspace(-0.004, 0.005, 101),
            np.linspace(-0.005, 0.004, 101),
        ),
        (
            np.random.default_rng(20260821).normal(0.0004, 0.012, 513),
            np.random.default_rng(20260822).normal(0.0003, 0.009, 513),
        ),
    ],
)
def test_down_market_capture_ratio_matches_empyrical(
    returns: np.ndarray, benchmark_returns: np.ndarray
) -> None:
...
def test_down_market_capture_ratio_uses_explicit_annualization() -> None:
    ...
    benchmark_cagr = float(
        empyrical.annual_return(benchmark[selected], annualization=12.0)
    )
    actual = DownMarketCaptureRatio.from_returns(
        returns, benchmark, periods_per_year=12.0
    ).compute()
    assert actual == pytest.approx(primary_cagr / benchmark_cagr, rel=1e-12)


def test_down_market_capture_ratio_factories_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.10, -0.20, 0.05])
    benchmark_returns = np.array([0.02, -0.10, -0.01])
    expected = DownMarketCaptureRatio.from_returns(
        returns, benchmark_returns, periods_per_year=12.0
    ).compute()

    assert DownMarketCaptureRatio.from_equity(
        np.array([100.0, 110.0, 88.0, 92.4]),
        np.array([200.0, 204.0, 183.6, 181.764]),
        periods_per_year=12.0,
    ).compute() == pytest.approx(expected)
    assert DownMarketCaptureRatio.from_pnl(
    ...
    assert DownMarketCaptureRatio.from_log_returns(
        np.log1p(returns),
        np.log1p(benchmark_returns),
        periods_per_year=12.0,
    ).compute() == pytest.approx(expected)

    state = DownMarketCaptureRatio.from_returns([], [], periods_per_year=12.0)
...
def test_down_market_capture_ratio_pairwise_missing_and_minimum() -> None:
    ...
    assert actual.compute() == pytest.approx(
        _empyrical_down_capture(returns, benchmark)
    )
    assert DownMarketCaptureRatio.from_returns([0.25], [-0.10]).compute() is not None
...
def test_down_market_capture_ratio_invalid_and_factory_contracts() -> None:
    with pytest.raises(ValueError):
        DownMarketCaptureRatio.from_returns([np.nan], [-0.01], nan_policy="raise")
    with pytest.raises(ValueError):
        DownMarketCaptureRatio.from_returns([np.inf], [-0.01])
    with pytest.raises(ValueError):
        DownMarketCaptureRatio.from_returns([0.01, 0.02], [-0.01])
    with pytest.raises(ValueError):
        DownMarketCaptureRatio.from_returns([0.01], [-0.02], periods_per_year=0.0)
