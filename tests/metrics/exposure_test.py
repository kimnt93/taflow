...
@pytest.mark.parametrize(
    "returns",
    [
        np.array([0.01, 0.0, 0.02, 0.0, 0.03]),
        np.array([0.01, 0.0, 0.0]),
        np.zeros(32),
        np.linspace(-0.04, 0.05, 101),
        np.array([0.02, -0.0, -0.01, 0.0, 0.02]),
        np.array([np.nan, 0.02, 0.0, np.nan, -0.01]),
        np.random.default_rng(20260812).choice(
            [0.0, -0.01, 0.01], size=513, p=[0.4, 0.3, 0.3]
        ),
    ],
)
...
def test_ceiling_to_percentage_point_is_intentionally_retained() -> None:
    ...
    assert Exposure.from_returns([0.01] + [0.0] * 100).compute() == pytest.approx(
        0.01
    )
...
@pytest.mark.parametrize("factory_name", ["from_returns", "from_positions"])
def test_lifecycle_missing_reset_and_cached_compute_are_invariant(
    factory_name: str,
) -> None:
    values = np.array([0.01, np.nan, 0.0, -0.02, 0.0, 0.03])
    factory = getattr(Exposure, factory_name)
    expected = factory(values).compute()
    state = factory([])
...
def test_missing_invalid_and_factory_contract() -> None:
    ...
    with pytest.raises(ValueError, match="NaN"):
        Exposure.from_positions([np.nan], nan_policy="raise")
