...
def _empyrical_up_down_capture(returns: np.ndarray, benchmark_returns: np.ndarray, periods_per_year: float=252.0) -> float | None:
    ...
    if periods_per_year == 252.0:
        result = float(empyrical.up_down_capture(primary, benchmark, period='daily'))
    else:
        up_capture = float(empyrical.annual_return(primary[up], annualization=periods_per_year) / empyrical.annual_return(benchmark[up], annualization=periods_per_year))
        down_capture = float(empyrical.annual_return(primary[down], annualization=periods_per_year) / empyrical.annual_return(benchmark[down], annualization=periods_per_year))
        result = up_capture / down_capture
    return result if np.isfinite(result) else None

@pytest.mark.parametrize(('returns', 'benchmark_returns'), [(np.array([0.01, -0.01]), np.array([0.02, -0.02])), (np.array([0.1, -0.2, 0.05, 0.01, -0.03]), np.array([0.02, -0.1, 0.0, 0.03, -0.02])), (np.array([np.nan, 0.02, -0.03, np.nan, 0.01, 0.005]), np.array([0.01, np.nan, 0.015, 0.02, -0.005, -0.002])), (np.linspace(-0.004, 0.005, 101), np.tile(np.array([-0.002, 0.0, 0.003, -0.003]), 26)[:101]), (np.random.default_rng(20260829).normal(0.0004, 0.012, 513), np.random.default_rng(20260830).normal(0.0003, 0.009, 513))])
def test_up_down_capture_ratio_matches_empyrical(returns: np.ndarray, benchmark_returns: np.ndarray) -> None:
...
def test_up_down_capture_ratio_uses_explicit_annualization() -> None:
    ...
    actual = UpDownCaptureRatio(periods_per_year=12.0).from_returns(returns, benchmark).compute()
...
def test_input_methods_and_lifecycle_are_invariant() -> None:
    ...
    expected = UpDownCaptureRatio(periods_per_year=12.0).from_returns(returns, benchmark_returns).compute()
    equity = np.array([100.0, 110.0, 88.0, 92.4])
    benchmark_equity = np.array([200.0, 204.0, 183.6, 185.436])
    assert UpDownCaptureRatio(periods_per_year=12.0).from_equity(equity, benchmark_equity).compute() == pytest.approx(expected)
    assert UpDownCaptureRatio(periods_per_year=12.0).from_pnl(np.array([10.0, -22.0, 4.4]), np.array([4.0, -20.4, 1.836]), initial_capital=100.0, benchmark_initial_capital=200.0).compute() == pytest.approx(expected)
    assert UpDownCaptureRatio(periods_per_year=12.0).from_log_returns(np.log1p(returns), np.log1p(benchmark_returns)).compute() == pytest.approx(expected)
    state = UpDownCaptureRatio(periods_per_year=12.0).from_returns([], [])
...
def test_pairwise_missing_filter_and_undefined_edges() -> None:
    ...
    assert len(actual) == 3
    assert actual.compute() == pytest.approx(_empyrical_up_down_capture(returns, benchmark))
    assert UpDownCaptureRatio().from_returns([0.01], [0.02]).compute() is None
    assert UpDownCaptureRatio().from_returns([0.02, 0.0], [0.01, -0.01]).compute() is None
...
def test_invalid_values_and_ambiguous_input_method_are_rejected() -> None:
    with pytest.raises(ValueError):
        UpDownCaptureRatio(nan_policy='raise').from_returns([np.nan], [0.01])
    with pytest.raises(ValueError):
        UpDownCaptureRatio().from_returns([np.inf], [0.01])
    with pytest.raises(ValueError):
        UpDownCaptureRatio().from_returns([0.01, -0.02], [0.01])
    with pytest.raises(ValueError):
        UpDownCaptureRatio(periods_per_year=0.0).from_returns([0.01], [-0.02])
    unbound = UpDownCaptureRatio()
    with pytest.raises(ValueError):
