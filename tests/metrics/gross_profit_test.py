...
@pytest.mark.parametrize('values', [np.array([100.0, -40.0, 0.0, 20.0, -10.0]), np.array([0.0, -20.0, -10.0]), np.array([25.0]), np.linspace(-100.0, 100.0, 101), np.array([np.nan, 100.0, -20.0, np.nan, 25.0]), np.random.default_rng(20260811).normal(40.0, 500.0, 513)])
...
def test_gross_profit_trade_input_method_preserves_absolute_values() -> None:
...
def test_gross_profit_boundaries_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        GrossProfit(nan_policy='raise').from_pnl([np.nan])
    with pytest.raises(ValueError):
