...
@pytest.mark.parametrize(("values", "cutoff"), [
    (np.array([-0.02, 0.01, 0.03, -0.01, 0.015]), 0.05),
    (np.linspace(-0.04, 0.05, 101), 0.01),
    (np.random.default_rng(20260812).normal(0.0004, 0.012, 513), 0.10),
    (np.array([np.nan, 0.02, -0.03, np.nan, 0.01]), 0.025),
])
def test_matches_scipy_gaussian_expected_shortfall(values, cutoff):
    ...
    actual = ParametricExpectedShortfall.from_returns(values, cutoff=cutoff).compute()
    assert actual == pytest.approx(expected, rel=2e-8, abs=2e-10)


def test_factories_and_streaming_lifecycle():
    ...
    assert ParametricExpectedShortfall.from_pnl(np.diff(equity), initial_equity=100.0).compute() == pytest.approx(expected)
...
def test_validation_and_constant_edge():
    ...
    with pytest.raises(ValueError):
        ParametricExpectedShortfall.from_returns([0.01, -0.01], cutoff=0.0)
    with pytest.raises(ValueError):
