...
@pytest.mark.parametrize(
    ...
    [
        np.array([0.10, -0.20]),
        np.array([0.10, 0.0, -0.20, 0.30, -0.10, 0.0]),
...
def test_kelly_criterion_freezes_decisive_breakeven_handling() -> None:
    ...
    with_breakevens = np.array([0.10, 0.0, -0.05, -0.0, 0.0])
...
def test_kelly_criterion_trade_factory_preserves_absolute_payoff_ratio() -> None:
...
def test_kelly_criterion_lifecycle_is_chunk_and_reset_invariant() -> None:
    values = np.array([0.10, np.nan, 0.0, -0.20, 0.30, -0.10])
...
def test_kelly_criterion_undefined_and_invalid_contract() -> None:
    ...
    with pytest.raises(ValueError):
        KellyCriterion.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_kelly_criterion_requires_semantic_factory() -> None:
