...
def quantstats_ulcer_index(returns: np.ndarray) -> float:
    ...
    series = pd.Series(returns, index=pd.date_range('2000-01-01', periods=len(returns), freq='D'))
    return float(qs.ulcer_index(series))

@pytest.mark.parametrize('returns', [np.array([0.1, -0.2]), np.zeros(32), np.linspace(-0.04, 0.05, 101), np.array([0.15, -0.1, -0.1, 0.25, -0.3, 0.1]), np.array([0.2, -0.2, 0.25, -0.2, 0.25, -0.2]), np.random.default_rng(20260811).normal(0.0004, 0.012, 513)])
...
def test_ulcer_index_input_methods_and_lifecycle_are_invariant() -> None:
    ...
    assert UlcerIndex().from_pnl(pnl, initial_capital=100.0).compute() == pytest.approx(expected)
    assert UlcerIndex().from_log_returns(np.log1p(returns)).compute() == pytest.approx(expected)
...
def test_ulcer_index_uses_quantstats_phantom_wealth_and_divisor() -> None:
    ...
    assert UlcerIndex().from_returns([0.1, -0.2, 0.25]).compute() == pytest.approx(expected)
...
def test_ulcer_index_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        UlcerIndex(nan_policy='raise').from_returns([np.nan])
    with pytest.raises(ValueError):
        UlcerIndex().from_returns([np.inf])
    with pytest.raises(ValueError):
        UlcerIndex().from_returns([-1.01])

def test_ulcer_index_requires_semantic_ingestion_before_append() -> None:
    with pytest.raises(ValueError):
        UlcerIndex().append(0.01)
