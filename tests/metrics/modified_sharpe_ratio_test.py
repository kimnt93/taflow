...
def _performanceanalytics_modified_sharpe(returns: np.ndarray, periods_per_year: float, annual_risk_free_rate: float, confidence_level: float) -> float | None:
    ...
    adjusted = z + (z ** 2 - 1.0) * skewness / 6.0 + (z ** 3 - 3.0 * z) * excess_kurtosis / 24.0 - (2.0 * z ** 3 - 5.0 * z) * skewness ** 2 / 36.0
...
@pytest.mark.parametrize(('returns', 'periods_per_year', 'annual_risk_free_rate', 'confidence_level'), [(np.array([0.02, -0.01, 0.03, -0.025, 0.01, -0.04, 0.03]), 252.0, 0.0, 0.95), (np.linspace(-0.04, 0.05, 101), 52.0, -0.01, 0.975), (np.array([0.15, -0.1, -0.1, 0.25, -0.3, 0.1]), 12.0, 0.04, 0.99), (np.array([np.nan, 0.02, -0.03, np.nan, 0.01]), 252.0, 0.03, 0.95), (np.random.default_rng(20260811).normal(0.0004, 0.012, 513), 365.0, 0.05, 0.9)])
def test_modified_sharpe_ratio_matches_pinned_source_translation(returns: np.ndarray, periods_per_year: float, annual_risk_free_rate: float, confidence_level: float) -> None:
    assert len(PERFORMANCE_ANALYTICS_2_1_0_SHA256) == 64
    actual = ModifiedSharpeRatio(periods_per_year=periods_per_year, annual_risk_free_rate=annual_risk_free_rate, confidence_level=confidence_level).from_returns(returns).compute()
    expected = _performanceanalytics_modified_sharpe(returns, periods_per_year, annual_risk_free_rate, confidence_level)
    if expected is None:
        assert actual is None
    else:
        assert actual == pytest.approx(expected, rel=2e-08, abs=2e-10)

def test_modified_sharpe_input_methods_and_lifecycle_are_invariant() -> None:
    returns = np.array([0.1, -0.2, 0.05, -0.03])
    settings = {'periods_per_year': 12.0, 'annual_risk_free_rate': 0.03, 'confidence_level': 0.975}
    expected = ModifiedSharpeRatio(**settings).from_returns(returns).compute()
    equity = 100.0 * np.r_[1.0, np.cumprod(1.0 + returns)]
    pnl = np.diff(equity)
    assert ModifiedSharpeRatio(**settings).from_equity(equity).compute() == pytest.approx(expected)
    assert ModifiedSharpeRatio(**settings).from_pnl(pnl, initial_capital=100.0).compute() == pytest.approx(expected)
    assert ModifiedSharpeRatio(**settings).from_log_returns(np.log1p(returns)).compute() == pytest.approx(expected)
    state = ModifiedSharpeRatio(**settings).from_returns([])
...
def test_modified_sharpe_boundaries_and_validation() -> None:
    ...
    with pytest.raises(ValueError):
        ModifiedSharpeRatio(nan_policy='raise').from_returns([np.nan])
    with pytest.raises(ValueError):
        ModifiedSharpeRatio().from_returns([np.inf])
    with pytest.raises(ValueError):
        ModifiedSharpeRatio().from_returns([-1.01])
    with pytest.raises(ValueError):
        ModifiedSharpeRatio(confidence_level=0.5).from_returns([0.01, -0.01])
    with pytest.raises(ValueError):
        ModifiedSharpeRatio(periods_per_year=0.0).from_returns([0.01, -0.01])

def test_modified_sharpe_requires_semantic_input_method() -> None:
    unbound = ModifiedSharpeRatio()
    with pytest.raises(ValueError):
        unbound.append(0.01)
