...
def test_recovery_factor_input_methods_and_lifecycle_are_invariant() -> None:
    ...
    assert RecoveryFactor().from_pnl(pnl, initial_capital=100.0).compute() == pytest.approx(expected)
    assert RecoveryFactor().from_log_returns(np.log1p(returns)).compute() == pytest.approx(expected)
...
def test_recovery_factor_undefined_missing_and_invalid_contract() -> None:
    ...
    assert RecoveryFactor().from_returns([0.1, 0.0, 0.2]).compute() is None
    assert RecoveryFactor().from_returns([-0.1, 0.1]).compute() == pytest.approx(0.0)
    assert len(RecoveryFactor().from_returns([np.nan, -0.01, np.nan])) == 1
    with pytest.raises(ValueError, match='NaN'):
        RecoveryFactor(nan_policy='raise').from_returns([np.nan])
    with pytest.raises(ValueError):
        RecoveryFactor().from_returns([np.inf])
    with pytest.raises(ValueError):
        RecoveryFactor().from_returns([-1.01])
    with pytest.raises(ValueError, match='initial_capital'):
        RecoveryFactor().from_pnl([1.0], initial_capital=0.0)

def test_recovery_factor_requires_semantic_input_method() -> None:
    metric = RecoveryFactor()
    with pytest.raises(ValueError):
        metric.append(0.01)
