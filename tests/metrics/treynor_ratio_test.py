...
def _performance_analytics_source_convention(
    ...
    if beta == 0.0 or not np.isfinite(beta):
        return None
    annualized_excess = (
        np.prod(1.0 + primary_excess) ** (periods_per_year / len(primary)) - 1.0
    )
...
def test_treynor_ratio_matches_quantstats_executable_cross_check() -> None:
    ...
    benchmark = pd.Series([0.05, -0.02, 0.02, 0.00, -0.01])
    periods = float(len(returns))
    expected = float(
        quantstats.stats.treynor_ratio(
            returns, benchmark, periods=periods, rf=0.0
        )
    )
    actual = TreynorRatio.from_returns(
        returns, benchmark, periods_per_year=periods
    ).compute()
    assert actual == pytest.approx(expected, rel=1e-12, abs=1e-14)


@pytest.mark.parametrize(
    ("returns", "benchmark", "periods_per_year", "annual_risk_free_rate"),
    [
        (
            np.array([0.08, -0.03, 0.04, 0.01, -0.02]),
            np.array([0.05, -0.02, 0.02, 0.00, -0.01]),
            12.0,
            0.06167781186449828,
        ),
        (
            np.array([np.nan, 0.02, -0.03, 0.01, 0.04]),
            np.array([0.01, np.nan, -0.01, 0.005, 0.02]),
            52.0,
            -0.01,
        ),
        (
...
def test_treynor_ratio_matches_pinned_performanceanalytics_source_convention(
    returns: np.ndarray,
    benchmark: np.ndarray,
    periods_per_year: float,
    annual_risk_free_rate: float,
) -> None:
    # This is source-specification parity, not a falsely labeled executable R match.
...
def test_treynor_ratio_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert TreynorRatio.from_log_returns(
        np.log1p(returns), np.log1p(benchmark_returns), **settings
    ).compute() == pytest.approx(expected)

    state = TreynorRatio.from_returns([], [], **settings)
    ...
    assert len(state) == 0
    assert state.extend(returns, benchmark_returns).compute() == pytest.approx(
        expected
    )
...
def test_treynor_ratio_pairwise_missing_minimum_and_zero_beta() -> None:
    ...
    state = TreynorRatio.from_returns(
        returns, benchmark, periods_per_year=1.0
    )
    assert len(state) == 2
    assert state.compute() == pytest.approx(
        _performance_analytics_source_convention(returns, benchmark, 1.0, 0.0)
    )
    assert TreynorRatio.from_returns([0.25], [0.10]).compute() is None
    assert TreynorRatio.from_returns([0.01, 0.01], [0.00, 0.02]).compute() is None
    assert TreynorRatio.from_returns([0.01, 0.02], [0.10, 0.10]).compute() is None


def test_treynor_ratio_rejects_invalid_input_without_partial_mutation() -> None:
    state = TreynorRatio.from_returns([0.01, 0.02], [0.00, 0.01])
    ...
    with pytest.raises(ValueError):
        TreynorRatio.from_returns([np.nan], [0.0], nan_policy="raise")
    with pytest.raises(ValueError):
        TreynorRatio.from_returns([np.inf], [0.0])
    with pytest.raises(ValueError):
        TreynorRatio.from_returns([0.01, 0.02], [0.01])
    with pytest.raises(ValueError):
        TreynorRatio.from_returns([0.01, 0.02], [0.00, 0.01], periods_per_year=0.0)
    with pytest.raises(ValueError):
        TreynorRatio.from_returns(
