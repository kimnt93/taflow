...
def test_total_return_factories_and_lifecycle_are_invariant() -> None:
    ...
    assert TotalReturn.from_pnl(pnl, initial_equity=100.0).compute() == pytest.approx(expected)
...
def test_total_return_missing_and_invalid_values() -> None:
    ...
    with pytest.raises(ValueError):
        TotalReturn.from_returns([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
...
def test_total_return_requires_semantic_factory() -> None:
