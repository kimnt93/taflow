...
def _quantstats_component_expectancy(values: np.ndarray) -> float | None:
    ...
    average_win = float(
        qs.avg_win(
            series, aggregate=None, compounded=True, prepare_returns=False
        )
    )
    average_loss = float(
        qs.avg_loss(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        np.array([np.nan, 20.0, -30.0, np.nan, 0.0, 10.0]),
        np.random.default_rng(20260811).normal(4.0, 120.0, 513),
    ],
)
...
def test_expectancy_breakevens_use_all_observation_denominator() -> None:
    ...
    assert Expectancy.from_trades([100.0, -40.0, 0.0]).compute() == pytest.approx(
        20.0
    )


def test_expectancy_factories_and_lifecycle_are_invariant() -> None:
...
def test_expectancy_missing_invalid_and_factory_contract() -> None:
    ...
    with pytest.raises(ValueError):
        Expectancy.from_pnl([np.nan], nan_policy="raise")
    with pytest.raises(ValueError):
