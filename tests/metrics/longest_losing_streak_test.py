...
def test_matches_quantstats(values):
    usable=values[~np.isnan(values)];series=pd.Series(usable,index=pd.date_range("2000-01-01",periods=len(usable),freq="D"))
...
def test_domains_lifecycle_validation():
    values=np.array([-0.1,-0.2,0,0.2,-0.3]);expected=2
    ...
    metric=LongestLosingStreak.from_returns([]).append(values[0]).extend(values[1:]);assert metric.compute()==expected;assert metric.reset().extend(values).compute()==expected
    with pytest.raises(TypeError):LongestLosingStreak()
    with pytest.raises(ValueError):LongestLosingStreak.from_returns([np.inf])
