...
def test_win_rate_excludes_breakevens_from_quantstats_denominator() -> None:
    values = np.array([0.10, 0.0, -0.20, 0.30, 0.0])
    assert WinRate.from_returns(values).compute() == pytest.approx(2.0 / 3.0)
    assert WinRate.from_returns(values).compute() == pytest.approx(
        quantstats_win_rate(values)
    )
    assert len(WinRate.from_returns(values)) == values.size


@pytest.mark.parametrize("factory_name", ["from_pnl", "from_trades"])
def test_win_rate_raw_pnl_and_trade_factories(factory_name: str) -> None:
    values = np.array([100.0, 0.0, -25.0, 50.0, -10.0])
    factory = getattr(WinRate, factory_name)
    state = factory(values)
...
def test_win_rate_lifecycle_missing_and_reset_are_invariant() -> None:
    values = np.array([0.10, np.nan, 0.0, -0.20, 0.30, 0.0])
...
def test_win_rate_undefined_and_invalid_contract() -> None:
    ...
    assert WinRate.from_returns([-0.10, -0.20]).compute() == pytest.approx(0.0)
    assert len(WinRate.from_returns([np.nan, 0.0, np.nan])) == 1

    with pytest.raises(ValueError, match="NaN"):
        WinRate.from_returns([np.nan], nan_policy="raise")
...
def test_win_rate_requires_semantic_factory() -> None:
    with pytest.raises(TypeError):
        WinRate()
