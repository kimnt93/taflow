...
from taflow import MathSqrt


def test_math_sqrt_matches_talib_and_lifecycle() -> None:
    rng = np.random.default_rng(104729)
    primary = np.linspace(0.0, 64.0, 127)
    datasets = (
        primary[:1],
        primary,
    ...
    for values in datasets:
        expected = (talib.SQRT)(values)
        actual = MathSqrt().extend(values)
